object frmHotelReceptionReport: TfrmHotelReceptionReport
  Left = 481
  Top = 228
  Width = 656
  Height = 586
  BiDiMode = bdRightToLeft
  Caption = #1711#1586#1575#1585#1588' '#1705#1575#1585#1705#1585#1583' '#1607#1578#1604' '#1607#1575
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -16
  Font.Name = 'Koodak'
  Font.Style = []
  OldCreateOrder = False
  ParentBiDiMode = False
  Position = poMainFormCenter
  PixelsPerInch = 96
  TextHeight = 28
  object DBGrid1: TDBGrid
    Left = 0
    Top = 121
    Width = 640
    Height = 364
    Align = alClient
    DataSource = dsHotel
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
    TabOrder = 0
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -16
    TitleFont.Name = 'Koodak'
    TitleFont.Style = []
    Columns = <
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'HotelReceptionID'
        Title.Alignment = taCenter
        Title.Caption = #1705#1583
        Width = 80
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ReceptionistName'
        Title.Caption = #1606#1575#1605
        Width = 213
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'HotelName'
        Title.Alignment = taCenter
        Title.Caption = #1607#1578#1604
        Width = 232
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'NumberOfCustomer'
        Title.Alignment = taCenter
        Title.Caption = #1578#1593#1583#1575#1583
        Width = 87
        Visible = True
      end>
  end
  object Panel1: TPanel
    Left = 0
    Top = 485
    Width = 640
    Height = 62
    Align = alBottom
    TabOrder = 1
    object BtnReturn: TBitBtn
      Left = 26
      Top = 14
      Width = 87
      Height = 36
      Caption = #1576#1575#1586#1711#1588#1578
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      Glyph.Data = {
        }
    end
    object BtnPrint: TBitBtn
      Left = 125
      Top = 15
      Width = 108
      Height = 36
      Caption = #1670#1575#1662'  '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      OnClick = BtnPrintClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00}
    end
    object BitBtn1: TBitBtn
      Left = 253
      Top = 15
      Width = 108
      Height = 36
      Caption = #1662#1584#1610#1585#1588
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 2
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00}
    end
    object ExportToExcel: TBitBtn
      Left = 381
      Top = 15
      Width = 124
      Height = 36
      Caption = #1575#1585#1587#1575#1604' '#1576#1607' '#1575#1705#1587#1604
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 3
      OnClick = ExportToExcelClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00}
    end
    object btnSummery: TBitBtn
      Left = 509
      Top = 15
      Width = 124
      Height = 36
      Caption = #1575#1705#1587#1604' '#1576#1607' '#1578#1601#1705#1610#1705
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 4
      OnClick = btnSummeryClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00}
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 0
    Width = 640
    Height = 121
    Align = alTop
    TabOrder = 2
    object Label14: TLabel
      Left = 580
      Top = 15
      Width = 42
      Height = 26
      Caption = #1575#1586' '#1578#1575#1585#1610#1582':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label3: TLabel
      Left = 470
      Top = 15
      Width = 15
      Height = 26
      Caption = #1575#1604#1610
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label4: TLabel
      Left = 320
      Top = 14
      Width = 27
      Height = 27
      Caption = #1607#1578#1604':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label1: TLabel
      Left = 317
      Top = 52
      Width = 56
      Height = 27
      Caption = #1606#1575#1605' '#1705#1575#1585#1576#1585':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object edtStartDate: TMaskEdit
      Left = 489
      Top = 11
      Width = 90
      Height = 34
      Color = clInfoBk
      EditMask = '1000/00/00;1;_'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      MaxLength = 10
      ParentFont = False
      TabOrder = 0
      Text = '1   /  /  '
    end
    object edtEndDate: TMaskEdit
      Left = 380
      Top = 11
      Width = 88
      Height = 34
      Color = clInfoBk
      EditMask = '1000/00/00;1;_'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      MaxLength = 10
      ParentFont = False
      TabOrder = 1
      Text = '1   /  /  '
    end
    object BBtnAccept: TBitBtn
      Left = 6
      Top = 89
      Width = 629
      Height = 24
      Caption = #1580#1587#1578#1580#1608
      TabOrder = 2
      OnClick = BBtnAcceptClick
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
    end
    object edtHotelID: TEdit
      Left = 273
      Top = 12
      Width = 41
      Height = 34
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 3
    end
    object edtHotelName: TEdit
      Left = 98
      Top = 12
      Width = 176
      Height = 34
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 4
    end
    object btnHotel: TBitBtn
      Left = 62
      Top = 12
      Width = 35
      Height = 33
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 5
      OnClick = btnHotelClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00}
    end
    object edtUserID: TEdit
      Left = 273
      Top = 50
      Width = 41
      Height = 34
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 6
    end
    object edtUserName: TEdit
      Left = 98
      Top = 50
      Width = 176
      Height = 34
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 7
    end
    object btnUser: TBitBtn
      Left = 62
      Top = 50
      Width = 35
      Height = 33
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 8
      OnClick = btnUserClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00}
    end
  end
  object dsHotel: TDataSource
    DataSet = QReportHotel
    Left = 64
    Top = 152
  end
  object QHotel: TADOQuery
    Connection = DM.ADOConnection
    BeforeDelete = QHotelBeforeDelete
    Parameters = <>
    SQL.Strings = (
      
        'Declare @TH table( HotelReceptionID int, ReceptionName'#9'nvarchar(' +
        '100), HotelName'#9'nvarchar(100),'
      
        #9'NumberOfCustomer'#9'int, Mod1'#9'int, HotelID'#9'int, CardCount100'#9'int, ' +
        'CardCount50'#9'int, CurrentMod'#9'int)'
      ''
      
        'declare @StartReceptionID bigint, @EndReceptionID Bigint, @Start' +
        'ReceptionDate char(10), @EndtReceptionDate Char(10)'
      'set @StartReceptionDate = '#39'1391/06/01'#39
      'set @EndtReceptionDate = '#39'1391/06/31'#39
      
        'select @StartReceptionID=MIN(ReceptionID), @EndReceptionID = MAX' +
        '(ReceptionID) from TReception'
      
        'where ReceptionDate >= @StartReceptionDate and ReceptionDate <= ' +
        '@EndtReceptionDate'
      
        'INSERT INTO @TH (HotelReceptionID, ReceptionName, HotelName, Num' +
        'berOfCustomer, Mod1, HotelID)'
      
        'select THotelReception.HotelReceptionID, ReceptionistName, Hotel' +
        'Name, SUM(NumberOfAdult) as NumberOfCustomer, THotelReception.Mo' +
        'd1, THotel.HotelID'
      
        'from TOrderHotel inner join TReception on TOrderHotel.ReceptionI' +
        'D = TReception.ReceptionID'
      
        'INNER JOIN THotelReception  on THotelReception.HotelReceptionID ' +
        '= TOrderHotel.HotelReceptionID'
      'inner join THotel on THotel.HotelID = THotelReception.HotelID'
      
        'where TReception.ReceptionID >= @StartReceptionID and TReception' +
        '.ReceptionID <= @EndReceptionID'
      
        'Group by THotelReception.HotelReceptionID, ReceptionistName, Hot' +
        'elName, THotel.HotelID, THotelReception.Mod1'
      ''
      ''
      
        'update @TH set CardCount100 = (NumberOfCustomer+Mod1) / 30, Curr' +
        'entMod = NumberOfCustomer+Mod1'
      ''
      
        'update @TH set CardCount50 = ((NumberOfCustomer+Mod1) - cardCoun' +
        't100*30) / 15'
      ''
      
        'update @TH set CurrentMod = (NumberOfCustomer+Mod1) - CardCount1' +
        '00*30 - CardCount50*15'
      ''
      'select * from @TH order by THotel.HotelID')
    Left = 24
    Top = 160
  end
  object QReportHotel: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <>
    SQL.Strings = (
      
        'declare @StartReceptionID bigint, @EndReceptionID Bigint, @Start' +
        'ReceptionDate char(10), @EndtReceptionDate Char(10)'
      
        '    select @StartReceptionID=MIN(ReceptionID), @EndReceptionID =' +
        ' MAX(ReceptionID) from TReception'
      'where ReceptionDate = '#39'1391/05/29'#39
      
        '  select THotelReception.HotelReceptionID, HotelName, CustomerNa' +
        'me, SUM(NumberOfAdult) as NumberOfCustomer, THotel.HotelID, Star' +
        'tTime'
      
        '    from TOrderHotel inner join TReception on TOrderHotel.Recept' +
        'ionID = TReception.ReceptionID'
      
        '    INNER JOIN THotelReception  on THotelReception.HotelReceptio' +
        'nID = TOrderHotel.HotelReceptionID'
      
        '    inner join THotel on THotel.HotelID = THotelReception.HotelI' +
        'D'
      
        '    where TReception.ReceptionID >= @StartReceptionID and TRecep' +
        'tion.ReceptionID <= @EndReceptionID'
      
        '    group by THotelReception.HotelReceptionID, HotelName, THotel' +
        '.HotelID, CustomerName, StartTime'
      '    order by StartTime')
    Left = 376
    Top = 504
  end
  object QSummery: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <>
    Left = 576
    Top = 456
  end
end
