...
var
  frmAgeCategory: TfrmAgeCategory;
  StatusRecord:String;
...
{*******************  SetEnabledDisable  ***********************}
procedure TfrmAgeCategory.SetEnabledDisable(BL:Boolean);
Begin
    Grid.Enabled:=BL;
...
End;
{******************* LoadSqlQuery ***********************}
Procedure TfrmAgeCategory.LoadSqlQuery;
Begin
    QAgeCategory.Open;
End;

{*******************  GetValue  ***********************}
procedure TfrmAgeCategory.GetValue;
begin
    edtTitle.Text:=QAgeCategory.fieldByName('title').Text;
end;

{*******************  FormCreate  ***********************}
procedure TfrmAgeCategory.FormCreate(Sender: TObject);
begin
    LoadSqlQuery;
end;

{*******************  FormKeyDown  ***********************}
procedure TfrmAgeCategory.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
    IF (key=27)  Then
...
{*******************  GridKeyDown  ***********************}
procedure TfrmAgeCategory.GridKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
{*******************  BtnReturnClick  ***********************}
procedure TfrmAgeCategory.BtnReturnClick(Sender: TObject);
begin
...
{*******************  BtnSaveClick  ***********************}
procedure TfrmAgeCategory.BtnSaveClick(Sender: TObject);
begin
    ...
    Begin
        ...
        Exit;
    End;
    SP_AgeCategory.Parameters.ParamByName('@id').Value:=QAgeCategory['id'];
...
{*******************  BtnCancelClick  ***********************}
procedure TfrmAgeCategory.BtnCancelClick(Sender: TObject);
begin
    SetEnabledDisable(True);
end;

{*******************  BtnNewClick  ***********************}
procedure TfrmAgeCategory.BtnNewClick(Sender: TObject);
begin
    StatusRecord:='Insert';
...
{*******************  BtnEditClick  ***********************}
procedure TfrmAgeCategory.BtnEditClick(Sender: TObject);
begin
    IF (QAgeCategory.RecNo<=0) OR (QAgeCategory.IsEmpty) Then Exit;
    SetEnabledDisable(False);
    StatusRecord:='Update';
    GetValue;
end;
...
{*******************  BtnDelClick  ***********************}
procedure TfrmAgeCategory.BtnDelClick(Sender: TObject);
begin
  try
    IF ((QAgeCategory.RecNo<=0) OR (QAgeCategory.IsEmpty)) Or (MessageBox(Self.Handle,'¬Ì« „Ì ŒÊ«ÂÌœ «Ì‰ —ﬂÊ—œ Õ–› ‘Êœ ø','Õ–› —ﬂÊ—œ',MB_YESNO+MB_DEFBUTTON1+MB_ICONWARNING+MB_RIGHT)=mrNo)  Then Exit;
    QAgeCategory.Delete;
    Grid.SetFocus;
  except
...
{*******************  Form Show  ***********************}
procedure TfrmAgeCategory.FormShow(Sender: TObject);
begin
    Grid.SetFocus;
end;

{*******************  Form Close  ***********************}
procedure TfrmAgeCategory.FormClose(Sender: TObject; var Action: TCloseAction);
begin
    QAgeCategory.Close;
end;
