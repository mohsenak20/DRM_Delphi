object frmOrderSelect: TfrmOrderSelect
  Left = 195
  Top = 125
  Width = 827
  Height = 545
  BiDiMode = bdRightToLeft
  Caption = #1575#1606#1578#1582#1575#1576' '#1575#1605#1585#1610#1607' '#1608' '#1578#1593#1610#1610#1606' '#1578#1593#1583#1575#1583' '#1606#1601#1585#1575#1578' '#1580#1607#1578' '#1589#1583#1608#1585' '#1576#1604#1610#1591
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'Koodak'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  ParentBiDiMode = False
  Position = poDesktopCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 23
  object DBGrid1: TDBGrid
    Left = 0
    Top = 0
    Width = 811
    Height = 392
    Align = alClient
    DataSource = dsOrder
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
    TabOrder = 0
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -13
    TitleFont.Name = 'Koodak'
    TitleFont.Style = []
    Columns = <
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'OrderID'
        Title.Alignment = taCenter
        Title.Caption = #1588#1605#1575#1585#1607' '#1575#1605#1585#1610#1607
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PersonOrder'
        Title.Alignment = taCenter
        Title.Caption = #1605#1593#1585#1601
        Width = 151
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CustomerName'
        Title.Alignment = taCenter
        Title.Caption = #1606#1605#1575#1610#1606#1583#1607' '#1575#1587#1578#1601#1575#1583#1607' '#1705#1606#1606#1583#1607
        Width = 216
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'Adult'
        Title.Alignment = taCenter
        Title.Caption = #1576#1586#1585#1711#1587#1575#1604
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'Child'
        Title.Alignment = taCenter
        Title.Caption = #1582#1585#1583#1587#1575#1604
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'AdultDiscount'
        Title.Alignment = taCenter
        Title.Caption = #1578#1582#1601#1610#1601' '#1576#1586#1585#1711#1587#1575#1604
        Width = 90
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'ChildDiscount'
        Title.Alignment = taCenter
        Title.Caption = #1578#1582#1601#1610#1601' '#1582#1585#1583#1587#1575#1604
        Width = 78
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Comment'
        Title.Alignment = taCenter
        Title.Caption = #1588#1585#1581
        Width = 154
        Visible = True
      end>
  end
  object Panel1: TPanel
    Left = 0
    Top = 392
    Width = 811
    Height = 115
    Align = alBottom
    TabOrder = 1
    DesignSize = (
      811
      115)
    object Label23: TLabel
      Left = 717
      Top = 36
      Width = 76
      Height = 24
      Caption = #1578#1593#1583#1575#1583' '#1576#1586#1585#1711#1587#1575#1604
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label1: TLabel
      Left = 718
      Top = 68
      Width = 76
      Height = 24
      Caption = #1578#1593#1583#1575#1583' '#1582#1585#1583#1587#1575#1604':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object edtAdult: TEdit
      Left = 645
      Top = 32
      Width = 65
      Height = 31
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
    end
    object BtnEnter: TBitBtn
      Left = 552
      Top = 30
      Width = 66
      Height = 36
      Anchors = [akTop, akRight]
      Caption = #1578#1575#1610#1610#1583
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00}
    end
    object edtChild: TEdit
      Left = 645
      Top = 63
      Width = 65
      Height = 31
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 2
    end
    object BtnReturn: TBitBtn
      Left = 18
      Top = 62
      Width = 87
      Height = 36
      Caption = #1576#1575#1586#1711#1588#1578
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 3
      Glyph.Data = {
        }
    end
  end
  object dsOrder: TDataSource
    DataSet = QOrder
    Left = 48
    Top = 80
  end
  object QOrder: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'CurrentDate'
        Size = -1
        Value = Null
      end
      item
        Name = 'UserID'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'Declare @AdultPrice int, @ChildPrice int'
      
        'select @AdultPrice = Bozorgsal, @ChildPrice = Khordsal from TPar' +
        'ameter'
      ''
      
        'Select OrderID, PersonOrder, CustomerName, Adult, Child, AdultDi' +
        'scount,'
      'ChildDiscount, Comment, TOrder.UserName, Hostname, NetAddress,'
      'OrderTime, OrderDate, TOrder.UserID, TUser.UserName,'
      'Adult*AdultDiscount AdultsDiscount,'
      'Child*ChildDiscount ChildsDiscount,'
      '(@AdultPrice-AdultDiscount)*Adult AdultPrice,'
      '(@ChildPrice-ChildDiscount)*Child ChildPrice,'
      'Adult*AdultDiscount + Child*ChildDiscount Discount,'
      
        '((@AdultPrice-AdultDiscount)*Adult)+ ((@ChildPrice-ChildDiscount' +
        ')*Child) Price'
      'from TOrder inner join TUser on TOrder.UserID = TUser.UserID'
      
        'where (OrderDate = :CurrentDate and OrderID not in (select Order' +
        'NUmber from TReception where UserID <> :UserID and OrderNumber >' +
        ' 0))'
      '')
    Left = 48
    Top = 48
  end
end
