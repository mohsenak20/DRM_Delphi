object frmSpecialCreditCardReport: TfrmSpecialCreditCardReport
  Left = 478
  Top = 219
  Width = 814
  Height = 614
  BiDiMode = bdRightToLeft
  Caption = #1587#1575#1576#1602#1607' '#1705#1575#1585#1578' '#1575#1593#1578#1576#1575#1585#1610
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -16
  Font.Name = 'Koodak'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  ParentBiDiMode = False
  Position = poDesktopCenter
  PixelsPerInch = 96
  TextHeight = 28
  object DBGrid1: TDBGrid
    Left = 0
    Top = 0
    Width = 798
    Height = 496
    Align = alClient
    DataSource = dsReport
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
    TabOrder = 0
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -16
    TitleFont.Name = 'Koodak'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'creditcardChargeID'
        Title.Alignment = taCenter
        Title.Caption = #1705#1583
        Width = 50
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Name'
        Title.Alignment = taCenter
        Title.Caption = #1606#1575#1605
        Width = 183
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ChargeDate'
        Title.Alignment = taCenter
        Title.Caption = #1578#1575#1585#1610#1582
        Width = 102
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ChargeTime'
        Title.Alignment = taCenter
        Title.Caption = #1587#1575#1593#1578
        Width = 67
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Price'
        Title.Alignment = taCenter
        Title.Caption = #1605#1576#1604#1594
        Width = 97
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NumberOfSession'
        Title.Alignment = taCenter
        Title.Caption = #1578#1593#1583#1575#1583
        Width = 38
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Used'
        Title.Alignment = taCenter
        Title.Caption = #1575#1587#1578#1601#1575#1583#1607' '#1588#1583#1607
        Width = 76
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Mod'
        Title.Alignment = taCenter
        Title.Caption = #1605#1575#1606#1583#1607
        Width = 46
        Visible = True
      end>
  end
  object Panel1: TPanel
    Left = 0
    Top = 496
    Width = 798
    Height = 80
    Align = alBottom
    TabOrder = 1
    object BtnSave: TBitBtn
      Left = 134
      Top = 25
      Width = 82
      Height = 36
      Caption = #1575#1606#1578#1582#1575#1576
      TabOrder = 0
      Glyph.Data = {
        
        00000000000000000000}
    end
    object BtnReturn: TBitBtn
      Left = 20
      Top = 25
      Width = 87
      Height = 36
      Caption = #1576#1575#1586#1711#1588#1578
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      OnClick = BtnReturnClick
      Glyph.Data = {
        }
    end
  end
  object QReport: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'CreditCardID'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      
        'select creditcardChargeID, Name+'#39' '#39'+lastName name, ChargeDate, C' +
        'hargeTime, NumberOfSession, Price,'
      'CreditGroupName, TCReditGroup.CreditGroupID, NID,'
      
        '(select SUM(NumberOfAdult) from TReception where CreditCardID = ' +
        'cast(creditcardChargeID as nvarchar(15))) Used,'
      
        'ISnULL(NumberOfSession-(select SUM(NumberOfAdult) from TReceptio' +
        'n where CreditCardID = cast(creditcardChargeID as nvarchar(15)))' +
        ', NumberOfSession) Mod'
      
        'from TCreditCardCharge inner join TCreditCard on TCreditCard.Car' +
        'dID = TCreditCardCharge.CreditCardID'
      
        'inner join TCReditGroup on TCreditCardCharge.ChargeGroupID = TCR' +
        'editGroup.CreditGroupID'
      'where CreditCardID = :CreditCardID')
    Left = 296
    Top = 160
  end
  object dsReport: TDataSource
    DataSet = QReport
    Left = 224
    Top = 160
  end
end
