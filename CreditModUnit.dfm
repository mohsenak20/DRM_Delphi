object frmCreditMod: TfrmCreditMod
  Left = 93
  Top = 198
  Width = 448
  Height = 361
  BiDiMode = bdRightToLeft
  Caption = #1605#1575#1606#1583#1607' '#1588#1575#1585#1688
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -16
  Font.Name = 'Koodak'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  ParentBiDiMode = False
  Position = poDesktopCenter
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 28
  object Label13: TLabel
    Left = 332
    Top = 101
    Width = 93
    Height = 29
    Caption = #1588#1575#1585#1688' '#1576#1575#1602#1610#1605#1575#1606#1583#1607':'
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Traffic'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object lblCreditMod: TLabel
    Left = 264
    Top = 99
    Width = 53
    Height = 33
    Caption = #1662#1584#1610#1585#1588':'
    Font.Charset = ARABIC_CHARSET
    Font.Color = clTeal
    Font.Height = -19
    Font.Name = 'Koodak'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label14: TLabel
    Left = 371
    Top = 15
    Width = 30
    Height = 26
    Caption = #1578#1575#1585#1610#1582':'
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Koodak'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object lblStatus: TLabel
    Left = 311
    Top = 285
    Width = 106
    Height = 24
    Caption = #1604#1591#1601#1575' '#1705#1605#1610' '#1589#1576#1585' '#1606#1605#1575#1610#1610#1583
    Font.Charset = ARABIC_CHARSET
    Font.Color = clRed
    Font.Height = -13
    Font.Name = 'Traffic'
    Font.Style = [fsBold]
    ParentFont = False
    Visible = False
  end
  object Label1: TLabel
    Left = 284
    Top = 133
    Width = 141
    Height = 29
    Caption = #1588#1575#1585#1688' '#1576#1575#1602#1610#1605#1575#1606#1583#1607' '#1585#1582#1578#1705#1606'1:'
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Traffic'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object lblModCredit1: TLabel
    Left = 216
    Top = 131
    Width = 53
    Height = 33
    Caption = #1662#1584#1610#1585#1588':'
    Font.Charset = ARABIC_CHARSET
    Font.Color = clTeal
    Font.Height = -19
    Font.Name = 'Koodak'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label3: TLabel
    Left = 280
    Top = 165
    Width = 145
    Height = 29
    Caption = #1588#1575#1585#1688' '#1576#1575#1602#1610#1605#1575#1606#1583#1607' '#1585#1582#1578#1705#1606'2:'
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Traffic'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object lblModCredit2: TLabel
    Left = 216
    Top = 163
    Width = 53
    Height = 33
    Caption = #1662#1584#1610#1585#1588':'
    Font.Charset = ARABIC_CHARSET
    Font.Color = clTeal
    Font.Height = -19
    Font.Name = 'Koodak'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label2: TLabel
    Left = 365
    Top = 49
    Width = 51
    Height = 27
    Caption = #1605#1580#1605#1608#1593#1607':'
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Traffic'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label4: TLabel
    Left = 258
    Top = 237
    Width = 166
    Height = 29
    Caption = #1588#1575#1585#1688' '#1576#1575#1602#1610#1605#1575#1606#1583#1607' '#1585#1582#1578#1705#1606' '#1601#1575#1586'2:'
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Traffic'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object lblModCreditFaz2: TLabel
    Left = 198
    Top = 235
    Width = 53
    Height = 33
    Caption = #1662#1584#1610#1585#1588':'
    Font.Charset = ARABIC_CHARSET
    Font.Color = clTeal
    Font.Height = -19
    Font.Name = 'Koodak'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label5: TLabel
    Left = 250
    Top = 205
    Width = 174
    Height = 29
    Caption = #1588#1575#1585#1688' '#1576#1575#1602#1610#1605#1575#1606#1583#1607' '#1585#1582#1578#1705#1606' 1 '#1601#1575#1586'2:'
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Traffic'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object lblServiceCenter2_1: TLabel
    Left = 182
    Top = 203
    Width = 53
    Height = 33
    Caption = #1662#1584#1610#1585#1588':'
    Font.Charset = ARABIC_CHARSET
    Font.Color = clTeal
    Font.Height = -19
    Font.Name = 'Koodak'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object BtnReturn: TBitBtn
    Left = 18
    Top = 276
    Width = 89
    Height = 36
    Caption = #1576#1575#1586#1711#1588#1578
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Koodak'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 0
    OnClick = BtnReturnClick
    Glyph.Data = {
      }
  end
  object BBtnAccept: TBitBtn
    Left = 114
    Top = 276
    Width = 89
    Height = 36
    Caption = #1580#1587#1578#1580#1608
    TabOrder = 1
    OnClick = BBtnAcceptClick
    Glyph.Data = {
      
      0000}
    NumGlyphs = 2
  end
  object edtDate: TMaskEdit
    Left = 277
    Top = 11
    Width = 88
    Height = 34
    Color = clInfoBk
    EditMask = '1000/00/00;1;_'
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Koodak'
    Font.Style = [fsBold]
    MaxLength = 10
    ParentFont = False
    TabOrder = 2
    Text = '1   /  /  '
  end
  object edtService_center_id: TEdit
    Left = 319
    Top = 47
    Width = 41
    Height = 34
    Color = clInfoBk
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Koodak'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 3
  end
  object edtService_center: TEdit
    Left = 144
    Top = 47
    Width = 176
    Height = 34
    Color = clInfoBk
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Koodak'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 4
  end
  object btnService_center: TBitBtn
    Left = 107
    Top = 47
    Width = 35
    Height = 33
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Koodak'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 5
    OnClick = btnService_centerClick
    Glyph.Data = {
      
      EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00}
  end
  object QCreditMod: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'ReceptionDate'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      
        'declare @ReceptionDate char(10), @ModCharge1 bigint, @ModCharge2' +
        ' BigInt, @ModChargeFaz2 BigInt, @ModChargeFaz2_1 BigInt'
      'set @ReceptionDate= :ReceptionDate'
      ''
      ''
      ''
      ''
      
        'select @ModCharge1=(((Select Sum(cast(TCredit.Credit as bigint))' +
        ' from TReception inner join TCredit on TReception.ReceptionID=TC' +
        'Redit.ReceptionID where ReceptionDate= @ReceptionDate and ExitUs' +
        'erID is null and ClosedID >=1017 and ClosedID<=2630  and service' +
        '_center_id = 1)-'
      
        '(Select sum(cast(Number as bigint)*VahedPrice-Discount) from TRe' +
        'ception inner join TFactor on TReception.ReceptionID=TFactor.Rec' +
        'eptionID'
      'inner join TSubFactor on TFactor.FactorID=TSubFactor.FactorID'
      
        #9#9'where TReception.ReceptionDate=@ReceptionDate and ExitUserID i' +
        's null and ClosedID >=1017 and ClosedID<=2630  and service_cente' +
        'r_id = 1)))'
      'from TReception as TR'
      'where REceptionDate=@ReceptionDate'
      'group by ReceptionDate'
      ''
      
        'select @ModCharge2=(((Select Sum(cast(TCredit.Credit as bigint))' +
        ' from TReception inner join TCredit on TReception.ReceptionID=TC' +
        'Redit.ReceptionID where ReceptionDate= @ReceptionDate and ExitUs' +
        'erID is null and ((ClosedID >=1 and ClosedID<=1016) or (ClosedID' +
        ' >=2631 and ClosedID<=2946  and service_center_id = 1)))-'
      
        '(Select sum(Number*VahedPrice-Discount) from TReception inner jo' +
        'in TFactor on TReception.ReceptionID=TFactor.ReceptionID'
      'inner join TSubFactor on TFactor.FactorID=TSubFactor.FactorID'
      
        #9#9'where TReception.ReceptionDate=@ReceptionDate and ExitUserID i' +
        's null and ((ClosedID >=1 and ClosedID<=1016) or (ClosedID >=263' +
        '1 and ClosedID<=2946  and service_center_id = 1)))))'
      'from TReception as TR'
      'where REceptionDate=@ReceptionDate'
      'group by ReceptionDate'
      ''
      
        'select @ModChargeFaz2=(((Select Sum(cast(TCredit.Credit as bigin' +
        't)) from TReception inner join TCredit on TReception.ReceptionID' +
        '=TCRedit.ReceptionID where ReceptionDate= @ReceptionDate and Exi' +
        'tUserID is null and ClosedID >=5001)-'
      
        '(Select sum(cast(Number as bigint)*VahedPrice-Discount) from TRe' +
        'ception inner join TFactor on TReception.ReceptionID=TFactor.Rec' +
        'eptionID'
      'inner join TSubFactor on TFactor.FactorID=TSubFactor.FactorID'
      
        #9#9'where TReception.ReceptionDate=@ReceptionDate and ExitUserID i' +
        's null and ClosedID >=5001)))'
      'from TReception as TR'
      'where REceptionDate=@ReceptionDate'
      'group by ReceptionDate'
      ''
      ''
      
        'select @ModChargeFaz2_1=(((Select Sum(cast(TCredit.Credit as big' +
        'int)) from TReception inner join TCredit on TReception.Reception' +
        'ID=TCRedit.ReceptionID where ReceptionDate= @ReceptionDate and E' +
        'xitUserID is null and ClosedID >=1 and ClosedID<=3258  and servi' +
        'ce_center_id = 2)-'
      
        '(Select sum(cast(Number as bigint)*VahedPrice-Discount) from TRe' +
        'ception inner join TFactor on TReception.ReceptionID=TFactor.Rec' +
        'eptionID'
      'inner join TSubFactor on TFactor.FactorID=TSubFactor.FactorID'
      
        #9#9'where TReception.ReceptionDate=@ReceptionDate and ExitUserID i' +
        's null and ClosedID >=1 and ClosedID<=3258 and service_center_id' +
        ' = 2)))'
      'from TReception as TR'
      'where REceptionDate=@ReceptionDate'
      'group by ReceptionDate'
      ''
      'if @ModCharge1 is null set @ModCharge1 = 0'
      'if @ModCharge2 is null set @ModCharge2 = 0'
      'if @ModChargeFaz2 is null set @ModChargeFaz2 = 0'
      'if @ModChargeFaz2_1 is null set @ModChargeFaz2_1 = 0'
      ''
      
        'select @ModCharge1 ModCharge1, @ModCharge2 ModCharge2, @ModCharg' +
        'eFaz2 ModChargeFaz2,  @ModChargeFaz2_1 ModChargeFaz2_1, @ModChar' +
        'ge1+@ModCharge2+@ModChargeFaz2+@ModChargeFaz2_1 ModCharge')
    Left = 8
    Top = 132
  end
  object SP_ModCharge: TADOStoredProc
    Connection = DM.ADOConnection
    ProcedureName = 'SP_GetModCharge;1'
    Parameters = <
      item
        Name = '@RETURN_VALUE'
        DataType = ftInteger
        Direction = pdReturnValue
        Precision = 10
        Value = Null
      end
      item
        Name = '@Receptiondate'
        Attributes = [paNullable]
        DataType = ftString
        Size = 10
        Value = Null
      end>
    Left = 72
    Top = 132
  end
  object QMod: TADOQuery
    Connection = DM.ADOConnection
    CursorType = ctStatic
    Parameters = <
      item
        Name = 'ReceptionDate'
        Size = -1
        Value = Null
      end
      item
        Name = 'service_center_id'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      
        'declare @ReceptionDate char(10), @ModCharge1 bigint, @ModCharge2' +
        ' BigInt, @Charge bigint, @Factor bigint, @service_center_id int'
      'set @ReceptionDate= :ReceptionDate'
      'set @service_center_id = :service_center_id'
      ''
      
        'set @Charge= (Select Sum(TCredit.Credit) from TReception inner j' +
        'oin TCredit on TReception.ReceptionID=TCRedit.ReceptionID'
      
        'where ReceptionDate= @ReceptionDate and EndTime='#39'23:59'#39' and Exit' +
        'UserID is null and service_center_id = @service_center_id)'
      ''
      ''
      
        'set @Factor = (Select sum(Number*VahedPrice-Discount) from TRece' +
        'ption inner join TFactor on TReception.ReceptionID=TFactor.Recep' +
        'tionID'
      'inner join TSubFactor on TFactor.FactorID=TSubFactor.FactorID'
      
        #9#9'where TReception.ReceptionDate=@ReceptionDate and EndTime='#39'23:' +
        '59'#39' and ExitUserID is null  and service_center_id = @service_cen' +
        'ter_id)'
      ''
      'if @Charge is null'
      #9'set @Charge = 0'
      ''
      'if @Factor is null'
      #9'set @Factor = 0'
      ''
      'set @ModCharge1 = @Charge - @Factor'
      ''
      '  '
      'select @ModCharge1 as ModCharge'
      '')
    Left = 120
    Top = 132
  end
end
