...
type CRCCode = (CRC_CCITT, CRC_16, CRC_32);
var
  crcinit :LongWord ;
  polynom : LongWord;
  direct : Integer ;
  ...
  crcmask :Longint  ;
  crchighbit:Longint ;
  crcinit_direct : LongWord;
  crcinit_nondirect: LongWord;
  crctab : array[0..256] of LongWord ;
...
function Crc16_P(X: TByteArr; N: Integer; Crc: Word = 0): Word; overload;
...
function CheckCrc16_P(X: TByteArr; N: Integer; Crc: Word): Integer; overload;
...
// arbitrary polynom
function Crc16_P(const Crc16Table: TCrc16Table; X: TByteArr; N: Integer; Crc: Word = 0): Word; overload;
...
function CheckCrc16_P(const Crc16Table: TCrc16Table; X: TByteArr; N: Integer; Crc: Word): Integer; overload;
...
procedure  Init(CodingType : CRCCode);
function crctablefast(p: Array of Byte):LongWord;
Function CheckCRC(p: Array of Byte;CRCValue : Array of Byte) : Boolean;

// for backward compatibility (default polynom = IEEE = $04C11DB7)
function Crc32_P(X: TByteArr; N: Integer; Crc: Cardinal = 0): Cardinal; overload;
...
function CheckCrc32_P(X: TByteArr; N: Integer; Crc: Cardinal): Integer; overload;
...
// arbitrary polynom
function Crc32_P(const Crc32Table: TCrc32Table; X: TByteArr; N: Integer; Crc: Cardinal = 0): Cardinal; overload;
...
function CheckCrc32_P(const Crc32Table: TCrc32Table; X: TByteArr; N: Integer; Crc: Cardinal): Integer; overload;
...
function  reflect(crc : LongWord; bitnum : Integer):LongWord ;
var
 i, j :LongWord ;
 crcout :LongWord ;
begin
...
begin
     ...
     if (refin = 0) then
        begin
        i:= 0;
           while (i < countArray) do begin
            crc := (crc shl 8) xor crctab[((crc shr (order - 8)) and $ff) xor p[i]];
            i:= i+1;
           end;
        end
        else
            begin
                    for i := 0 to countArray do
                      crc := (crc shr 8) xor crctab[(crc and $ff) xor p[i]];
            end;
                if ((refout xor refin) <> 0) then
                   crc := reflect(crc, order);
        crc := crc xor crcxor;
        crc := crc and crcmask;
        Result := crc;
...
begin
   ...
   crcmask := (((1 shl (order - 1)) - 1) shl 1) or 1;
                crchighbit := 1 shl (order - 1);
...
Begin
   Result := False;
   Init(CRC_16);
   CALC_CRC := crctablefast(P);

   If (CRCValue[0] = Byte(StrToInt('$' +Mid(IntToHex(CALC_CRC,4),3,2)))) And
      (CRCValue[1] = Byte(StrToInt('$' +Mid(IntToHex(CALC_CRC,4),1,2)))) Then
       Result := True;
End;
...
function Crc16_P(const Crc16Table: TCrc16Table; X: TByteArr; N: Integer; Crc: Word): Word;
var
  I: Integer;
begin
...
function Crc16_P(X: TByteArr; N: Integer; Crc: Word): Word;
begin
  Result := Crc16_P(Crc16DefaultTable, X, N, Crc);
end;

function CheckCrc16_P(const Crc16Table: TCrc16Table; X: TByteArr; N: Integer; Crc: Word): Integer;
...
function CheckCrc16_P(X: TByteArr; N: Integer; Crc: Word): Integer;
begin
...
function Crc32_P(const Crc32Table: TCrc32Table; X: TByteArr; N: Integer; Crc: Cardinal): Cardinal;
var
  I: Integer;
begin
...
function Crc32_P(X: TByteArr; N: Integer; Crc: Cardinal): Cardinal;
begin
  Result := Crc32_P(Crc32DefaultTable, X, N, Crc);
end;

function CheckCrc32_P(const Crc32Table: TCrc32Table; X: TByteArr; N: Integer; Crc: Cardinal): Integer;
...
function CheckCrc32_P(X: TByteArr; N: Integer; Crc: Cardinal): Integer;
begin
