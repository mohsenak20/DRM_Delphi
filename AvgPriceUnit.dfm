object frmAvgPrice: TfrmAvgPrice
  Left = 186
  Top = 119
  Width = 1173
  Height = 746
  BiDiMode = bdRightToLeft
  Caption = #1602#1610#1605#1578' '#1605#1610#1575#1606#1711#1610#1606
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -16
  Font.Name = 'Koodak'
  Font.Style = []
  OldCreateOrder = False
  ParentBiDiMode = False
  Position = poDesktopCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 28
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1157
    Height = 121
    Align = alTop
    TabOrder = 0
    DesignSize = (
      1157
      121)
    object Label1: TLabel
      Left = 1091
      Top = 17
      Width = 43
      Height = 28
      Anchors = [akTop, akRight]
      Caption = #1575#1586' '#1578#1575#1585#1610#1582':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clTeal
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = []
      ParentFont = False
    end
    object Label3: TLabel
      Left = 971
      Top = 17
      Width = 16
      Height = 28
      Anchors = [akTop, akRight]
      Caption = #1575#1604#1610
    end
    object lblAvgPrice: TLabel
      Left = 102
      Top = 9
      Width = 43
      Height = 28
      Anchors = [akTop, akRight]
      Caption = #1575#1586' '#1578#1575#1585#1610#1582':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clTeal
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = []
      ParentFont = False
    end
    object Label4: TLabel
      Left = 154
      Top = 9
      Width = 74
      Height = 28
      Caption = #1602#1610#1605#1578' '#1605#1610#1575#1606#1711#1610#1606':'
    end
    object Label2: TLabel
      Left = 657
      Top = 52
      Width = 115
      Height = 28
      Caption = #1705#1575#1585#1578#1607#1575#1610' '#1662#1584#1610#1585#1588' '#1588#1583#1607':'
    end
    object lblSumCardPrice: TLabel
      Left = 284
      Top = 52
      Width = 43
      Height = 28
      Anchors = [akTop, akRight]
      Caption = #1575#1586' '#1578#1575#1585#1610#1582':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clTeal
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = []
      ParentFont = False
    end
    object Label6: TLabel
      Left = 337
      Top = 52
      Width = 147
      Height = 28
      Caption = #1602#1610#1605#1578' '#1705#1575#1585#1578#1607#1575#1610' '#1662#1584#1610#1585#1588' '#1588#1583#1607':'
    end
    object lblSumCardCount: TLabel
      Left = 605
      Top = 52
      Width = 43
      Height = 28
      Anchors = [akTop, akRight]
      Caption = #1575#1586' '#1578#1575#1585#1610#1582':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clTeal
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = []
      ParentFont = False
    end
    object Label8: TLabel
      Left = 79
      Top = 52
      Width = 109
      Height = 28
      Caption = #1605#1610#1575#1606#1711#1610#1606' '#1602#1610#1605#1578' '#1705#1575#1585#1578#1607#1575':'
    end
    object lblMidPrice: TLabel
      Left = 27
      Top = 52
      Width = 43
      Height = 28
      Anchors = [akTop, akRight]
      Caption = #1575#1586' '#1578#1575#1585#1610#1582':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clTeal
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = []
      ParentFont = False
    end
    object BBtnAccept: TBitBtn
      Left = 16
      Top = 92
      Width = 1129
      Height = 25
      Anchors = [akTop, akRight]
      TabOrder = 2
      OnClick = BBtnAcceptClick
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
    end
    object edtStartDate: TMaskEdit
      Left = 992
      Top = 13
      Width = 99
      Height = 37
      Anchors = [akTop, akRight]
      Color = clInfoBk
      EditMask = '1000/00/00;1;_'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      MaxLength = 10
      ParentFont = False
      TabOrder = 0
      Text = '1   /  /  '
    end
    object edtEndDate: TMaskEdit
      Left = 871
      Top = 13
      Width = 97
      Height = 37
      Anchors = [akTop, akRight]
      Color = clInfoBk
      EditMask = '1000/00/00;1;_'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      MaxLength = 10
      ParentFont = False
      TabOrder = 1
      Text = '1   /  /  '
    end
    object rbAvgPrice: TRadioButton
      Left = 656
      Top = 15
      Width = 113
      Height = 17
      Caption = #1602#1610#1605#1578' '#1605#1610#1575#1606#1711#1610#1606
      Checked = True
      TabOrder = 3
      TabStop = True
    end
    object rbDiscount: TRadioButton
      Left = 504
      Top = 15
      Width = 113
      Height = 17
      Caption = #1578#1582#1601#1610#1601#1575#1578' '#1575#1605#1585#1610#1607
      TabOrder = 4
    end
    object rbDailyInCome: TRadioButton
      Left = 392
      Top = 15
      Width = 73
      Height = 17
      Caption = #1662#1584#1610#1585#1588
      TabOrder = 5
    end
    object rbNumber: TRadioButton
      Left = 264
      Top = 15
      Width = 97
      Height = 17
      Caption = #1578#1593#1583#1575#1583' '#1662#1584#1610#1585#1588
      TabOrder = 6
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 658
    Width = 1157
    Height = 49
    Align = alBottom
    TabOrder = 1
    object btnReturn: TBitBtn
      Left = 9
      Top = 8
      Width = 87
      Height = 36
      Caption = #1576#1575#1586#1711#1588#1578
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      OnClick = btnReturnClick
      Glyph.Data = {
        }
    end
    object btnPrint: TBitBtn
      Left = 113
      Top = 8
      Width = 87
      Height = 36
      Caption = #1670#1575#1662' '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00}
    end
  end
  object Panel3: TPanel
    Left = 0
    Top = 121
    Width = 1157
    Height = 537
    Align = alClient
    TabOrder = 2
    object Chart1: TChart
      Left = 1
      Top = 1
      Width = 1155
      Height = 535
      BackWall.Brush.Color = clWhite
      BackWall.Brush.Style = bsClear
      Gradient.EndColor = 8421631
      Gradient.Visible = True
      Title.Font.Charset = ARABIC_CHARSET
      Title.Font.Color = clBlack
      Title.Font.Height = -16
      Title.Font.Name = 'Koodak'
      Title.Font.Style = [fsBold]
      Title.Text.Strings = (
        #1606#1605#1608#1583#1575#1585' '#1602#1610#1605#1578' '#1605#1610#1575#1606#1711#1610#1606)
      LeftAxis.LabelsFont.Charset = ARABIC_CHARSET
      LeftAxis.LabelsFont.Color = clBlack
      LeftAxis.LabelsFont.Height = -13
      LeftAxis.LabelsFont.Name = 'Arial'
      LeftAxis.LabelsFont.Style = [fsBold]
      LeftAxis.Title.Font.Charset = ARABIC_CHARSET
      LeftAxis.Title.Font.Color = clBlack
      LeftAxis.Title.Font.Height = -13
      LeftAxis.Title.Font.Name = 'Koodak'
      LeftAxis.Title.Font.Style = [fsBold]
      Legend.Font.Charset = ARABIC_CHARSET
      Legend.Font.Color = clBlack
      Legend.Font.Height = -13
      Legend.Font.Name = 'Koodak'
      Legend.Font.Style = [fsBold]
      Legend.Visible = False
      View3D = False
      Align = alClient
      TabOrder = 0
      object Series1: TLineSeries
        Marks.ArrowLength = 8
        Marks.Visible = False
        SeriesColor = 16711808
        Pointer.HorizSize = 2
        Pointer.InflateMargins = True
        Pointer.Style = psRectangle
        Pointer.VertSize = 2
        Pointer.Visible = True
        XValues.DateTime = False
        XValues.Name = 'X'
        XValues.Multiplier = 1.000000000000000000
        XValues.Order = loAscending
        YValues.DateTime = False
        YValues.Name = 'Y'
        YValues.Multiplier = 1.000000000000000000
        YValues.Order = loNone
      end
      object Series2: TLineSeries
        Marks.ArrowLength = 8
        Marks.Visible = False
        SeriesColor = clRed
        Pointer.InflateMargins = True
        Pointer.Style = psRectangle
        Pointer.Visible = False
        XValues.DateTime = False
        XValues.Name = 'X'
        XValues.Multiplier = 1.000000000000000000
        XValues.Order = loAscending
        YValues.DateTime = False
        YValues.Name = 'Y'
        YValues.Multiplier = 1.000000000000000000
        YValues.Order = loNone
      end
      object Series3: TLineSeries
        Marks.ArrowLength = 8
        Marks.Visible = False
        SeriesColor = clGreen
        Pointer.InflateMargins = True
        Pointer.Style = psRectangle
        Pointer.Visible = False
        XValues.DateTime = False
        XValues.Name = 'X'
        XValues.Multiplier = 1.000000000000000000
        XValues.Order = loAscending
        YValues.DateTime = False
        YValues.Name = 'Y'
        YValues.Multiplier = 1.000000000000000000
        YValues.Order = loNone
      end
    end
  end
  object QOrderDiscount: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'StartDate'
        DataType = ftWideString
        NumericScale = 255
        Precision = 255
        Size = 10
        Value = Null
      end
      item
        Name = 'EndDate'
        DataType = ftWideString
        NumericScale = 255
        Precision = 255
        Size = 10
        Value = Null
      end>
    SQL.Strings = (
      'select ReceptionDate, Price, AvrPrice, '
      
        'AvrPrice*(select SUM(NumberOfAdult) from TReception where Recept' +
        'ionDate = TOD.ReceptionDate) as DailyInCome,'
      
        '(select SUM(NumberOfAdult) from TReception where ReceptionDate =' +
        ' TOD.ReceptionDate) as Number'
      'from TOrderDiscount TOD'
      'where ReceptionDate >= :StartDate and ReceptionDate<= :EndDate'
      'and AvrPrice > 0')
    Left = 24
    Top = 24
  end
  object QAvgPrice: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'StartReceptionDate'
        Size = -1
        Value = Null
      end
      item
        Name = 'EndReceptionDate'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      
        'select cast(SUM(AvrPrice)/COUNT(ReceptionDate) as int) as avg fr' +
        'om TOrderDiscount'
      
        'where ReceptionDate >= :StartReceptionDate and ReceptionDate <= ' +
        ':EndReceptionDate')
    Left = 152
    Top = 40
  end
  object QCard: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <>
    SQL.Strings = (
      
        'Declare @CardPrice1 bigint, @CardPrice2 bigint,'#9'@CardPrice3 bigi' +
        'nt, @CardCount bigint,@CardCount2 bigint, @CardCount3 bigint'
      'Declare @SumPrice bigint, @SumCardCount int, @MidPrice int'
      
        'select @CardPrice1=SUM(cast(TCard.Credit as bigint)), @CardCount' +
        ' = COUNT(TCard.Credit) from TReception inner join TCard on TRece' +
        'ption.CardID = TCard.CardID'
      'inner join TBargain on TCard.BargainID = TBargain.BargainID'
      
        'where ReceptionDate >= '#39'1390/11/01'#39' and ReceptionDate <= '#39'1390/1' +
        '1/30'#39' and TReception.CardID > 4 and GetMoney = 0'
      ''
      
        'select @CardPrice2=SUM(cast(TCard.Credit as bigint)), @CardCount' +
        '2 = COUNT(TCard.Credit) from TReception inner join TCard on TRec' +
        'eption.CardID2 = TCard.CardID'
      'inner join TBargain on TCard.BargainID = TBargain.BargainID'
      
        'where ReceptionDate >= '#39'1390/11/01'#39' and ReceptionDate <= '#39'1390/1' +
        '1/30'#39' and TReception.CardID2 > 4 and GetMoney = 0'
      ''
      
        'select @CardPrice3=SUM(cast(TCard.Credit as bigint)), @CardCount' +
        '3 = COUNT(TCard.Credit) from TReception inner join TCard on TRec' +
        'eption.CardID3 = TCard.CardID'
      'inner join TBargain on TCard.BargainID = TBargain.BargainID'
      
        'where ReceptionDate >= '#39'1390/11/01'#39' and ReceptionDate <= '#39'1390/1' +
        '1/30'#39' and TReception.CardID3 > 4 and GetMoney = 0'
      'set @SumPrice = @CardPrice1+@CardPrice2+@CardPrice3'
      'set @SumCardCount = @CardCount+ @CardCount2 + @CardCount3'
      'set @MidPrice = @SumPrice / @SumCardCount'
      
        'select @SumPrice SumCardPrice, @SumCardCount SumCardCount, @MidP' +
        'rice MidPrice')
    Left = 344
    Top = 96
  end
end
