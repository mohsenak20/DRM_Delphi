object frmReceptionSummaryReport: TfrmReceptionSummaryReport
  Left = 194
  Top = 184
  Width = 1305
  Height = 675
  BiDiMode = bdRightToLeft
  Caption = #1711#1586#1575#1585#1588' '#1587#1585#1580#1605#1593' '#1601#1585#1608#1588
  Color = clBtnFace
  ...
  object DBGrid1: TDBGrid
    ...
    Columns = <
      ...
      item
        ...
        FieldName = 'intenet'
        Title.Alignment = taCenter
        Title.Caption = #1575#1610#1606#1578#1585#1606#1578
        Width = 147
        Visible = True
      end
      item
      ...
      item
        ...
        FieldName = 'receipt'
        Title.Alignment = taCenter
        Title.Caption = #1601#1585#1608#1588' '#1662#1610#1605#1575#1606#1705#1575#1585#1575#1606
        Width = 172
        Visible = True
      end>
  end
  object QSummary: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <>
    SQL.Strings = (
      'select receptionDate, sum(cast(price as bigint)) price,'
      
        '(select sum(cast(TCredit.credit as bigint)) from TReception inne' +
        'r join TCredit on TReception.ReceptionID = TCredit.ReceptionID w' +
        'here  ReceptionDate = TR.ReceptionDate) credit,'
      
        '(select sum((cast(NumberOfAdult as bigint)*vahedPrice)+(cast(Num' +
        'berOfChild as bigint)*child_price)) from TReception inner join t' +
        'factors on TReception.order_id = tfactors.id'
      'where ReceptionDate = TR.ReceptionDate) intenet,'
      '(select sum(cast(credit as bigint)) from VReceptionCard'
      'where ReceptionDate = TR.ReceptionDate and GetMoney = 0) card,'
      
        '(select sum(NumberOfAdult*HotelTickets.adultPrice+NumberOfChild*' +
        'HotelTickets.childPrice) from TReception inner join HotelTickets' +
        ' on TReception.hotelTicketId = HotelTickets.id'
      'where ReceptionDate = TR.ReceptionDate) hotel,'
      
        '(select sum(cast(TSubFactor.Number as bigint)*VahedPrice) from T' +
        'Reception inner join TFactor on TReception.ReceptionID =TFactor.' +
        'ReceptionID'
      'inner join TSubFactor on TFactor.FactorID = TSubFactor.FactorID'
      'where ReceptionDate = TR.ReceptionDate) receipt'
      'from TReception TR'
