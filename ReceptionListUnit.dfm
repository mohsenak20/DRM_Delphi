object frmReceptionList: TfrmReceptionList
  Left = 328
  Top = 235
  Width = 834
  Height = 559
  BiDiMode = bdRightToLeft
  Caption = #1662#1584#1610#1585#1588' '
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -16
  Font.Name = 'Koodak'
  Font.Style = []
  KeyPreview = True
  Menu = MainMenuUnit
  OldCreateOrder = False
  ParentBiDiMode = False
  Position = poScreenCenter
  OnCreate = FormCreate
  DesignSize = (
    818
    500)
  PixelsPerInch = 96
  TextHeight = 28
  object PanelButton: TPanel
    Left = 0
    Top = 453
    Width = 818
    Height = 47
    Align = alBottom
    BevelInner = bvLowered
    TabOrder = 0
    DesignSize = (
      818
      47)
    object Label7: TLabel
      Left = 450
      Top = 10
      Width = 58
      Height = 29
      Caption = #1576#1586#1585#1711#1587#1575#1604':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
      Visible = False
    end
    object lblBozorgsal: TLabel
      Left = 351
      Top = 11
      Width = 96
      Height = 29
      Caption = #1578#1593#1583#1575#1583' '#1576#1586#1585#1711#1587#1575#1604':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clMaroon
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
      Visible = False
    end
    object lblKhordsal: TLabel
      Left = 226
      Top = 10
      Width = 96
      Height = 29
      Caption = #1578#1593#1583#1575#1583' '#1576#1586#1585#1711#1587#1575#1604':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clMaroon
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
      Visible = False
    end
    object Label10: TLabel
      Left = 326
      Top = 10
      Width = 56
      Height = 29
      Caption = #1582#1585#1583#1587#1575#1604':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
      Visible = False
    end
    object lblSumReception: TLabel
      Left = 126
      Top = 7
      Width = 105
      Height = 34
      Caption = #1578#1593#1583#1575#1583' '#1576#1586#1585#1711#1587#1575#1604':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clPurple
      Font.Height = -20
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      Visible = False
    end
    object Label9: TLabel
      Left = 234
      Top = 10
      Width = 28
      Height = 29
      Caption = #1580#1605#1593':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
      Visible = False
    end
    object BtnEdit: TBitBtn
      Left = 716
      Top = 8
      Width = 87
      Height = 36
      Anchors = [akTop, akRight]
      Caption = #1608#1610#1585#1575#1610#1588
      Enabled = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      Visible = False
      OnClick = BtnEditClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00}
    end
    object BtnDel: TBitBtn
      Left = 621
      Top = 8
      Width = 87
      Height = 36
      Anchors = [akTop, akRight]
      Caption = #1581#1584#1601
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 2
      OnClick = BtnDelClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00D8E9EC00}
    end
    object BtnReturn: TBitBtn
      Left = 10
      Top = 6
      Width = 87
      Height = 36
      Caption = #1576#1575#1586#1711#1588#1578
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      OnClick = BtnReturnClick
      Glyph.Data = {
        }
    end
    object BtnPrint: TBitBtn
      Left = 524
      Top = 7
      Width = 87
      Height = 36
      Anchors = [akTop, akRight]
      Caption = #1670#1575#1662' '
      Enabled = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 3
      Visible = False
      OnClick = BtnPrintClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00}
    end
  end
  object StatusBar: TStatusBar
    Left = 0
    Top = 434
    Width = 818
    Height = 19
    Anchors = [akRight, akBottom]
    BiDiMode = bdRightToLeft
    Color = clInfoBk
    Panels = <
      item
        Text = #1576#1575#1586#1711#1588#1578' : Esc'
        Width = 139
      end
      item
        Text = #1670#1575#1662': Ctrl+P'
        Width = 139
      end
      item
        Text = #1581#1584#1601':  Del'
        Width = 139
      end
      item
        Text = #1608#1610#1585#1575#1610#1588':   Ctrl+E'
        Width = 139
      end
      item
        Text = #1583#1585#1580':  Insert'
        Width = 139
      end>
    ParentBiDiMode = False
  end
  object Grid: TDBGrid
    Left = 0
    Top = 121
    Width = 818
    Height = 313
    TabStop = False
    Align = alClient
    BiDiMode = bdRightToLeft
    DataSource = dsReception
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'koodak'
    Font.Style = [fsBold]
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection]
    ParentBiDiMode = False
    ParentFont = False
    ParentShowHint = False
    PopupMenu = PopupMenuUnit
    ShowHint = False
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'ReceptionID'
        Title.Caption = #1705#1583' '#1662#1584#1610#1585#1588
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ClosedID'
        Title.Caption = #1588#1605#1575#1585#1607' '#1705#1605#1583
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 62
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CustomerName'
        Title.Caption = #1606#1575#1605
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 176
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CustomerType'
        Title.Caption = #1606#1608#1593
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 81
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ReceptionDate'
        Title.Caption = #1578#1575#1585#1610#1582
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ReceptionType'
        Title.Caption = #1606#1608#1593' '#1662#1584#1610#1585#1588
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 72
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'StartTime'
        Title.Caption = #1608#1585#1608#1583
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 53
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CardID'
        Title.Caption = #1705#1575#1585#1578'1'
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 99
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CardID2'
        Title.Caption = #1705#1575#1585#1578'2'
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 90
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CardID3'
        Title.Caption = #1705#1575#1585#1578'3'
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 100
        Visible = True
      end>
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 818
    Height = 121
    Align = alTop
    TabOrder = 3
    DesignSize = (
      818
      121)
    object Label4: TLabel
      Left = 697
      Top = 11
      Width = 21
      Height = 29
      Anchors = [akLeft, akTop, akRight]
      Caption = #1606#1575#1605':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label5: TLabel
      Left = 696
      Top = 51
      Width = 94
      Height = 29
      Anchors = [akLeft, akTop, akRight]
      Caption = #1588#1605#1575#1585#1607' '#1662#1584#1610#1585#1588':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label12: TLabel
      Left = 119
      Top = 54
      Width = 94
      Height = 29
      Caption = #1578#1593#1583#1575#1583' '#1582#1585#1583#1587#1575#1604':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lblNumberOfChild: TLabel
      Left = 17
      Top = 55
      Width = 96
      Height = 29
      Caption = #1578#1593#1583#1575#1583' '#1576#1586#1585#1711#1587#1575#1604':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clMaroon
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label14: TLabel
      Left = 357
      Top = 54
      Width = 96
      Height = 29
      Caption = #1578#1593#1583#1575#1583' '#1576#1586#1585#1711#1587#1575#1604':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lblNumberOfAdult: TLabel
      Left = 257
      Top = 55
      Width = 96
      Height = 29
      Caption = #1578#1593#1583#1575#1583' '#1576#1586#1585#1711#1587#1575#1604':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clMaroon
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object edtSCustomerName: TEdit
      Left = 528
      Top = 9
      Width = 163
      Height = 37
      Anchors = [akLeft, akTop, akRight]
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
    end
    object edtReceptionID: TEdit
      Left = 528
      Top = 49
      Width = 163
      Height = 37
      Anchors = [akLeft, akTop, akRight]
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
    end
    object BBtnAccept: TBitBtn
      Left = 16
      Top = 91
      Width = 797
      Height = 24
      Anchors = [akLeft, akTop, akRight]
      Caption = #1580#1587#1578#1580#1608
      TabOrder = 2
      OnClick = BBtnAcceptClick
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
    end
    object chbCard: TCheckBox
      Left = 96
      Top = 18
      Width = 97
      Height = 17
      Caption = #1705#1575#1585#1578#1610
      TabOrder = 3
    end
  end
  object GroupBox: TGroupBox
    Left = 197
    Top = 121
    Width = 616
    Height = 312
    Anchors = [akRight, akBottom]
    Caption = #1605#1588#1582#1589#1575#1578' '
    Color = clBtnFace
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Traffic'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    TabOrder = 4
    Visible = False
    DesignSize = (
      616
      312)
    object Label2: TLabel
      Left = 561
      Top = 38
      Width = 21
      Height = 29
      Caption = #1606#1575#1605':'
      Enabled = False
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label1: TLabel
      Left = 561
      Top = 83
      Width = 24
      Height = 29
      Caption = #1606#1608#1593':'
      Enabled = False
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label6: TLabel
      Left = 245
      Top = 35
      Width = 43
      Height = 29
      Anchors = [akLeft, akTop, akRight]
      Caption = #1705#1575#1585#1578'1:'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label8: TLabel
      Left = 245
      Top = 77
      Width = 47
      Height = 29
      Anchors = [akLeft, akTop, akRight]
      Caption = #1705#1575#1585#1578'2:'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label11: TLabel
      Left = 244
      Top = 117
      Width = 48
      Height = 29
      Anchors = [akLeft, akTop, akRight]
      Caption = #1705#1575#1585#1578'3:'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object BtnSave: TBitBtn
      Left = 16
      Top = 24
      Width = 75
      Height = 36
      Caption = #1584#1582#1610#1585#1607
      TabOrder = 0
      OnClick = BtnSaveClick
      Glyph.Data = {
        
        00000000000000000000}
    end
    object BtnCancel: TBitBtn
      Left = 16
      Top = 72
      Width = 75
      Height = 36
      Caption = #1575#1606#1589#1585#1575#1601
      TabOrder = 1
      OnClick = BtnCancelClick
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
    end
    object gb2: TGroupBox
      Left = 131
      Top = 153
      Width = 465
      Height = 153
      Caption = #1575#1605#1575#1606#1575#1578
      Enabled = False
      TabOrder = 2
      object Label3: TLabel
        Left = 398
        Top = 107
        Width = 48
        Height = 29
        Caption = #1605#1578#1601#1585#1602#1607':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object chbMobile: TCheckBox
        Left = 384
        Top = 29
        Width = 61
        Height = 17
        Caption = #1605#1608#1576#1575#1610#1604
        TabOrder = 0
      end
      object chbPool: TCheckBox
        Left = 336
        Top = 29
        Width = 45
        Height = 17
        Caption = #1662#1608#1604
        TabOrder = 1
      end
      object chbSaat: TCheckBox
        Left = 216
        Top = 29
        Width = 61
        Height = 17
        Caption = #1587#1575#1593#1578
        TabOrder = 2
      end
      object chbAngoshtar: TCheckBox
        Left = 84
        Top = 29
        Width = 97
        Height = 17
        Caption = #1575#1606#1711#1588#1578#1585' '#1608' '#1662#1604#1575#1603
        TabOrder = 3
      end
      object chbSooeech: TCheckBox
        Left = 16
        Top = 29
        Width = 61
        Height = 17
        Caption = #1587#1608#1574#1610#1670
        TabOrder = 4
      end
      object chbAsnad: TCheckBox
        Left = 392
        Top = 69
        Width = 53
        Height = 17
        Caption = #1575#1587#1606#1575#1583
        TabOrder = 5
      end
      object chbPakhsh: TCheckBox
        Left = 288
        Top = 69
        Width = 93
        Height = 17
        Caption = #1662#1582#1588' '#1575#1578#1608#1605#1576#1610#1604
        TabOrder = 6
      end
      object chbKeefPool: TCheckBox
        Left = 200
        Top = 69
        Width = 77
        Height = 17
        Caption = #1603#1610#1601' '#1662#1608#1604
        TabOrder = 7
      end
      object edtOther: TEdit
        Left = 24
        Top = 104
        Width = 369
        Height = 37
        Color = clInfoBk
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Koodak'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 9
      end
      object chbSamsonet: TCheckBox
        Left = 80
        Top = 69
        Width = 101
        Height = 17
        Caption = #1587#1575#1605#1587#1608#1606#1578
        TabOrder = 8
      end
    end
    object edtCustomerName: TEdit
      Left = 328
      Top = 33
      Width = 225
      Height = 37
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 3
    end
    object rbBozorgsal: TRadioButton
      Left = 480
      Top = 89
      Width = 73
      Height = 17
      Caption = #1576#1586#1585#1711#1587#1575#1604
      Checked = True
      Enabled = False
      TabOrder = 4
      TabStop = True
    end
    object rbKhordsal: TRadioButton
      Left = 400
      Top = 89
      Width = 73
      Height = 17
      Caption = #1582#1585#1583#1587#1575#1604
      Enabled = False
      TabOrder = 5
    end
    object edtCard1: TEdit
      Left = 143
      Top = 33
      Width = 98
      Height = 37
      Anchors = [akLeft, akTop, akRight]
      Color = clInfoBk
      Enabled = False
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 6
    end
    object edtCard2: TEdit
      Left = 143
      Top = 73
      Width = 98
      Height = 37
      Anchors = [akLeft, akTop, akRight]
      Color = clInfoBk
      Enabled = False
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 7
    end
    object edtCard3: TEdit
      Left = 143
      Top = 113
      Width = 98
      Height = 37
      Anchors = [akLeft, akTop, akRight]
      Color = clInfoBk
      Enabled = False
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 8
    end
  end
  object MainMenuUnit: TMainMenu
    Left = 16
    Top = 319
    object MnuFile: TMenuItem
      Caption = #1662#1585#1608#1606#1583#1607
      object MnuEdit: TMenuItem
        Caption = #1608#1610#1585#1575#1610#1588
        OnClick = BtnEditClick
      end
      object MnuDelete: TMenuItem
        Caption = #1581#1584#1601
        OnClick = BtnDelClick
      end
      object N2: TMenuItem
        Caption = '-'
      end
      object MnuReturn: TMenuItem
        Caption = #1576#1575#1586#1711#1588#1578
        OnClick = BtnReturnClick
      end
    end
  end
  object PopupMenuUnit: TPopupMenu
    Left = 49
    Top = 319
    object MnuPopEdit: TMenuItem
      Caption = #1608#1610#1585#1575#1610#1588
      OnClick = BtnEditClick
    end
    object N3: TMenuItem
      Caption = '-'
    end
    object mnuChangeKeyNumber: TMenuItem
      Caption = #1578#1594#1610#1610#1585' '#1583#1587#1578#1610' '#1705#1604#1610#1583
      OnClick = mnuChangeKeyNumberClick
    end
    object MnuPopNone1: TMenuItem
      Caption = '-'
    end
    object mnuChange: TMenuItem
      Caption = #1578#1576#1583#1610#1604' '#1576#1607
      OnClick = mnuChangeClick
    end
    object N1: TMenuItem
      Caption = '-'
    end
    object MnuPopDelete: TMenuItem
      Caption = #1581#1584#1601
      OnClick = BtnDelClick
    end
  end
  object SP_Reception: TADOStoredProc
    Connection = DM.ADOConnection
    ProcedureName = 'SP_TReception;1'
    Parameters = <
      item
        Name = '@RETURN_VALUE'
        DataType = ftInteger
        Direction = pdReturnValue
        Precision = 10
        Value = Null
      end
      item
        Name = '@ReceptionID'
        Attributes = [paNullable]
        DataType = ftLargeint
        Precision = 19
        Value = Null
      end
      item
        Name = '@CustomerName'
        Attributes = [paNullable]
        DataType = ftWideString
        Size = 100
        Value = Null
      end
      item
        Name = '@CustomerType'
        Attributes = [paNullable]
        DataType = ftSmallint
        Precision = 5
        Value = Null
      end
      item
        Name = '@ReceptionDate'
        Attributes = [paNullable]
        DataType = ftString
        Size = 10
        Value = Null
      end
      item
        Name = '@ReceptionType'
        Attributes = [paNullable]
        DataType = ftSmallint
        Precision = 5
        Value = Null
      end
      item
        Name = '@Shift'
        Attributes = [paNullable]
        DataType = ftSmallint
        Precision = 5
        Value = Null
      end
      item
        Name = '@ClosedID'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@StartTime'
        Attributes = [paNullable]
        DataType = ftString
        Size = 5
        Value = Null
      end
      item
        Name = '@EndTime'
        Attributes = [paNullable]
        DataType = ftString
        Size = 5
        Value = Null
      end
      item
        Name = '@Credit'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@Active'
        Attributes = [paNullable]
        DataType = ftBoolean
        Value = Null
      end
      item
        Name = '@Mobile'
        Attributes = [paNullable]
        DataType = ftBoolean
        Value = Null
      end
      item
        Name = '@Pool'
        Attributes = [paNullable]
        DataType = ftBoolean
        Value = Null
      end
      item
        Name = '@Saat'
        Attributes = [paNullable]
        DataType = ftBoolean
        Value = Null
      end
      item
        Name = '@AngoshtarPlak'
        Attributes = [paNullable]
        DataType = ftBoolean
        Value = Null
      end
      item
        Name = '@Sooeech'
        Attributes = [paNullable]
        DataType = ftBoolean
        Value = Null
      end
      item
        Name = '@Asnad'
        Attributes = [paNullable]
        DataType = ftBoolean
        Value = Null
      end
      item
        Name = '@Pakhsh'
        Attributes = [paNullable]
        DataType = ftBoolean
        Value = Null
      end
      item
        Name = '@KeefPool'
        Attributes = [paNullable]
        DataType = ftBoolean
        Value = Null
      end
      item
        Name = '@Samsonet'
        Attributes = [paNullable]
        DataType = ftBoolean
        Value = Null
      end
      item
        Name = '@Other'
        Attributes = [paNullable]
        DataType = ftWideString
        Size = 200
        Value = Null
      end
      item
        Name = '@Price'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@CardID'
        Attributes = [paNullable]
        DataType = ftLargeint
        Precision = 19
        Value = Null
      end
      item
        Name = '@CardID2'
        Attributes = [paNullable]
        DataType = ftLargeint
        Precision = 19
        Value = Null
      end
      item
        Name = '@CardID3'
        Attributes = [paNullable]
        DataType = ftLargeint
        Precision = 19
        Value = Null
      end
      item
        Name = '@Action'
        Attributes = [paNullable]
        DataType = ftWideString
        Size = 6
        Value = Null
      end>
    Left = 85
    Top = 319
  end
  object dsReception: TDataSource
    DataSet = QReception
    Left = 120
    Top = 320
  end
  object QReception: TADOQuery
    Connection = DM.ADOConnection
    CursorType = ctStatic
    BeforeDelete = QReceptionBeforeDelete
    OnCalcFields = QReceptionCalcFields
    Parameters = <
      item
        Name = 'UserID'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end
      item
        Name = 'ReceptionDate'
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 10
        Value = Null
      end>
    SQL.Strings = (
      
        'Select * From TReception inner join TClosed on TReception.Closed' +
        'Id = TClosed.ClosedId'
      'where UserID= :UserID and  ReceptionDate >= :ReceptionDate'
      'order by ReceptionID DESC'
      '')
    Left = 156
    Top = 320
    object QReceptionReceptionID: TLargeintField
      FieldName = 'ReceptionID'
    end
    object QReceptionCustomerName: TWideStringField
      FieldName = 'CustomerName'
      Size = 100
    end
    object QReceptionCustomerType: TSmallintField
      FieldName = 'CustomerType'
    end
    object QReceptionReceptionDate: TStringField
      FieldName = 'ReceptionDate'
      FixedChar = True
      Size = 10
    end
    object QReceptionReceptionType: TSmallintField
      FieldName = 'ReceptionType'
    end
    object QReceptionShift: TSmallintField
      FieldName = 'Shift'
    end
    object QReceptionStartTime: TStringField
      FieldName = 'StartTime'
      FixedChar = True
      Size = 5
    end
    object QReceptionEndTime: TStringField
      FieldName = 'EndTime'
      FixedChar = True
      Size = 5
    end
    object QReceptionCredit: TIntegerField
      FieldName = 'Credit'
    end
    object QReceptionActive: TBooleanField
      FieldName = 'Active'
    end
    object QReceptionMobile: TBooleanField
      FieldName = 'Mobile'
    end
    object QReceptionPool: TBooleanField
      FieldName = 'Pool'
    end
    object QReceptionSaat: TBooleanField
      FieldName = 'Saat'
    end
    object QReceptionAngoshtarPlak: TBooleanField
      FieldName = 'AngoshtarPlak'
    end
    object QReceptionSooeech: TBooleanField
      FieldName = 'Sooeech'
    end
    object QReceptionAsnad: TBooleanField
      FieldName = 'Asnad'
    end
    object QReceptionPakhsh: TBooleanField
      FieldName = 'Pakhsh'
    end
    object QReceptionKeefPool: TBooleanField
      FieldName = 'KeefPool'
    end
    object QReceptionSamsonet: TBooleanField
      FieldName = 'Samsonet'
    end
    object QReceptionOther: TWideStringField
      FieldName = 'Other'
      Size = 200
    end
    object QReceptionPrice: TIntegerField
      FieldName = 'Price'
    end
    object QReceptionCardID: TLargeintField
      FieldName = 'CardID'
    end
    object QReceptionUserID: TIntegerField
      FieldName = 'UserID'
    end
    object QReceptionEnter: TBooleanField
      FieldName = 'Enter'
    end
    object QReceptionstrMobile: TStringField
      FieldKind = fkCalculated
      FieldName = 'strMobile'
      Size = 1
      Calculated = True
    end
    object QReceptionstrPool: TStringField
      FieldKind = fkCalculated
      FieldName = 'strPool'
      Size = 1
      Calculated = True
    end
    object QReceptionstrSaat: TStringField
      FieldKind = fkCalculated
      FieldName = 'strSaat'
      Size = 1
      Calculated = True
    end
    object QReceptionstrAngoshtarPlak: TStringField
      FieldKind = fkCalculated
      FieldName = 'strAngoshtarPlak'
      Size = 1
      Calculated = True
    end
    object QReceptionstrSooeech: TStringField
      FieldKind = fkCalculated
      FieldName = 'strSooeech'
      Size = 1
      Calculated = True
    end
    object QReceptionstrAsnad: TStringField
      FieldKind = fkCalculated
      FieldName = 'strAsnad'
      Size = 1
      Calculated = True
    end
    object QReceptionstrPakhsh: TStringField
      FieldKind = fkCalculated
      FieldName = 'strPakhsh'
      Size = 1
      Calculated = True
    end
    object QReceptionstrKeefPool: TStringField
      FieldKind = fkCalculated
      FieldName = 'strKeefPool'
      Size = 1
      Calculated = True
    end
    object QReceptionstrSamsonet: TStringField
      FieldKind = fkCalculated
      FieldName = 'strSamsonet'
      Size = 1
      Calculated = True
    end
    object QReceptionClosedID: TIntegerField
      FieldName = 'ClosedID'
    end
    object QReceptionCardID2: TLargeintField
      FieldName = 'CardID2'
    end
    object QReceptionCardID3: TLargeintField
      FieldName = 'CardID3'
    end
    object QReceptionExitUserID: TIntegerField
      FieldName = 'ExitUserID'
    end
    object QReceptionNumberOfAdult: TIntegerField
      FieldName = 'NumberOfAdult'
    end
    object QReceptionNumberOfChild: TIntegerField
      FieldName = 'NumberOfChild'
    end
    object QReceptionCheckedCard1: TBooleanField
      FieldName = 'CheckedCard1'
    end
    object QReceptionCheckedCard2: TBooleanField
      FieldName = 'CheckedCard2'
    end
    object QReceptionCheckedCard3: TBooleanField
      FieldName = 'CheckedCard3'
    end
    object QReceptionCustomerExit1: TBooleanField
      FieldName = 'CustomerExit1'
    end
    object QReceptionCustomerExit2: TBooleanField
      FieldName = 'CustomerExit2'
    end
    object QReceptionExitTime1: TStringField
      FieldName = 'ExitTime1'
      FixedChar = True
      Size = 5
    end
    object QReceptionExitTime2: TStringField
      FieldName = 'ExitTime2'
      FixedChar = True
      Size = 5
    end
    object QReceptionEnterTime: TWideStringField
      FieldName = 'EnterTime'
      FixedChar = True
      Size = 5
    end
    object QReceptionReceptionDiscount: TIntegerField
      FieldName = 'ReceptionDiscount'
    end
    object QReceptionOrderNumber: TIntegerField
      FieldName = 'OrderNumber'
    end
    object QReceptionFactorNumber: TIntegerField
      FieldName = 'FactorNumber'
    end
    object QReceptionAdultPrice: TIntegerField
      FieldName = 'AdultPrice'
    end
    object QReceptionChildPrice: TIntegerField
      FieldName = 'ChildPrice'
    end
    object QReceptionkindergarten: TBooleanField
      FieldName = 'kindergarten'
    end
    object QReceptionTicketType: TIntegerField
      FieldName = 'TicketType'
    end
    object QReceptionHostName: TWideStringField
      FieldName = 'HostName'
      FixedChar = True
      Size = 40
    end
    object QReceptionNetAddress: TWideStringField
      FieldName = 'NetAddress'
      FixedChar = True
      Size = 12
    end
    object QReceptionCreditCardID: TWideStringField
      FieldName = 'CreditCardID'
      Size = 10
    end
    object QReceptionorder_id: TIntegerField
      FieldName = 'order_id'
    end
    object QReceptionmobile_number: TWideStringField
      FieldName = 'mobile_number'
      FixedChar = True
      Size = 13
    end
    object QReceptionservice_center_id: TIntegerField
      FieldName = 'service_center_id'
    end
    object QReceptionwarmDelete: TBooleanField
      FieldName = 'warmDelete'
    end
  end
  object ActionList: TActionList
    Left = 8
    Top = 360
    object ActionEdit: TAction
      Caption = 'ActionEdit'
      Enabled = False
      ShortCut = 16453
    end
    object ActionDel: TAction
      Caption = 'ActionDel'
      ShortCut = 46
      OnExecute = BtnDelClick
    end
    object ActionReturn: TAction
      Caption = 'ActionReturn'
      ShortCut = 27
      OnExecute = BtnReturnClick
    end
  end
  object RvProject: TRvProject
    Engine = RvSystem
    ProjectFile = 'EnterVer.rav'
    Left = 19
    Top = 205
  end
  object RvSystem: TRvSystem
    TitleSetup = 'Output Options'
    TitleStatus = 'Report Status'
    TitlePreview = 'Report Preview'
    SystemSetups = [ssAllowCopies, ssAllowCollate, ssAllowDuplex, ssAllowDestPreview, ssAllowDestPrinter, ssAllowDestFile, ssAllowPrinterSetup, ssAllowPreviewSetup]
    SystemFiler.StatusFormat = 'Generating page %p'
    SystemPreview.ZoomFactor = 100.000000000000000000
    SystemPrinter.Orientation = poLandScape
    SystemPrinter.ScaleX = 100.000000000000000000
    SystemPrinter.ScaleY = 100.000000000000000000
    SystemPrinter.StatusFormat = 'Printing page %p'
    SystemPrinter.Title = 'ReportPrinter Report'
    SystemPrinter.UnitsFactor = 1.000000000000000000
    Left = 59
    Top = 205
  end
  object QPrint: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'ReceptionID'
        Attributes = [paSigned]
        DataType = ftLargeint
        Precision = 19
        Size = 8
        Value = Null
      end>
    SQL.Strings = (
      
        'select Treception.ReceptionID, CustomerName, CustomerType, Recep' +
        'tionDate,'
      'TReception.NumberOfAdult, TReception.NumberOfChild,'
      
        'ReceptionType, Shift, StartTime, EndTime, TReception.Active, Mob' +
        'ile, Pool, Saat,'
      
        'AngoshtarPlak, Sooeech, Asnad, Pakhsh, KeefPool, Samsonet, Other' +
        ', Price,'
      
        'Treception.UserID as ReceptionUser, Enter, TCard.CardID, CardID2' +
        ', CardID3, BarCode, CardType, CreateDate, ExpireDate,'
      
        'TClosed.ClosedID, KeyNumber, PositionClosed, CompanyName, CardTy' +
        'peName, TClosed.Code, KinderGarten'
      
        'from TReception inner join TCard on TReception.CardID=TCard.Card' +
        'ID'
      'inner join TCardType on TCard.CardType=TCardType.CardTypeID'
      'inner join TClosed on TReception.ClosedID=TClosed.ClosedID'
      'inner join TBargain on TCard.BargainID=TBargain.BargainID'
      'inner join TCompany on TBargain.CompanyID=TCompany.CompanyID'
      'where TReception.ReceptionID = :ReceptionID')
    Left = 16
    Top = 232
  end
  object QCredit: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'ReceptionID'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'Select * from TCredit where ReceptionID= :ReceptionID')
    Left = 16
    Top = 280
  end
  object QTedad: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'UserID'
        Size = -1
        Value = Null
      end
      item
        Name = 'Receptiondate'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'Declare @UserID int'
      'Declare @ReceptionDate Char(10)'
      'set @UserID= :UserID'
      'set @ReceptionDate= :Receptiondate'
      
        'Select Count(ReceptionID) as Bozorgsal, (Select Count(ReceptionI' +
        'D) from TReception'
      
        'where UserID= @UserID and ReceptionDate= @ReceptionDate and Cust' +
        'omerType=2) as Khordsal,'
      
        '(select Sum(Price) from TReception where UserID= @UserID and Rec' +
        'eptionDate= @ReceptionDate) as SumReception'
      'from TReception'
      
        'where UserID= @UserID and ReceptionDate= @ReceptionDate and Cust' +
        'omerType=1')
    Left = 48
    Top = 280
  end
  object QDelete: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'ReceptionID'
        Attributes = [paSigned]
        DataType = ftLargeint
        Precision = 19
        Size = 8
        Value = Null
      end
      item
        Name = 'UserID'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      
        'declare @ReceptionID bigint, @CheckReceptionID bigint, @Delete i' +
        'nt, @UserID int, @CheckUserID int, @ExitUserID int, @PayMoney in' +
        't'
      'set @PayMoney = 0'
      'set @ReceptionID = :REceptionID'
      'set @UserID = :UserID'
      'select @CheckReceptionID = ReceptionID from TCredit'
      'where ReceptionID = @ReceptionID'
      ''
      'if @CheckReceptionID is not null'
      'begin'
      #9'set @CheckReceptionID = -1'
      
        #9'select @CheckReceptionID = Tfactor.ReceptionID from TReception ' +
        'inner join TFactor on TReception.ReceptionID = TFactor.Reception' +
        'ID'
      #9'where Tfactor.ReceptionID = @ReceptionID and ExitUserID is null'
      ''
      #9'if @CheckReceptionID <> -1'
      #9#9'set @Delete = 2'
      #9'else'
      #9'begin'
      #9#9'select @CheckUserID = UserID from TCredit'
      #9#9'where UserID <> @UserID and ReceptionID = @ReceptionID'
      #9#9'if @CheckUserID is null'
      '    begin'
      
        '      select @ExitUserID = ExitUserID from TReception where Rece' +
        'ptionID = @ReceptionID'
      '      if @ExitUserID is null'
      #9#9#9'  set @Delete = 1'
      '      else'
      '        set @Delete = 4'
      '    end'
      #9#9'else'
      '    begin'
      
        '      select @ExitUserID = ExitUserID from TReception where Rece' +
        'ptionID = @ReceptionID'
      '      if @ExitUserID is null'
      #9#9#9'  set @Delete = 3'
      '      else'
      '        set @Delete = 4'
      '    end;'
      #9'end'
      'end'
      'else'
      #9'set @Delete = 1'
      ''
      ''
      ''
      'if @Delete = 1'
      'begin'
      '  select @PayMoney = sum(Credit) from TCredit'
      '  where ReceptionID = @ReceptionID'
      '  if @PayMoney is null'
      '    set @PayMoney = 0'
      ''
      '  select @PayMoney = @PayMoney + Price from TReception'
      '    where ReceptionID = @ReceptionID'
      ''
      ''
      #9'delete TReception'
      #9'where ReceptionID = @ReceptionID'
      ''
      #9'select 1 as Result, @PayMoney as PayMoney'
      ''
      'end'
      'else'
      #9'select @Delete as Result')
    Left = 88
    Top = 280
  end
  object SP_DelReception: TADOStoredProc
    Connection = DM.ADOConnection
    ProcedureName = 'SP_DelReception;1'
    Parameters = <
      item
        Name = '@RETURN_VALUE'
        DataType = ftInteger
        Direction = pdReturnValue
        Precision = 10
        Value = Null
      end
      item
        Name = '@ReceptionID'
        Attributes = [paNullable]
        DataType = ftLargeint
        Precision = 19
        Value = Null
      end>
    Left = 56
    Top = 360
  end
  object QNumberOfReception: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'REceptionDate'
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 10
        Value = Null
      end
      item
        Name = 'UserID'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end
      item
        Name = 'CustomerName'
        DataType = ftWideString
        NumericScale = 255
        Precision = 255
        Size = 100
        Value = Null
      end>
    SQL.Strings = (
      
        'Select sum(NumberOfAdult) as NumberOfAdult, sum(NumberOfChild) a' +
        's NumberOfChild from TReception'
      
        'where ReceptionDate =  :ReceptionDate and UserID = :UserID and C' +
        'ustomerName like :CustomerName')
    Left = 24
    Top = 16
  end
  object QWarmDelete: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'ReceptionID'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'declare @ReceptionID bigint'
      'set @ReceptionID = :ReceptionID'
      ''
      
        'declare @UserName nchar(40), @NetAddress nchar(12), @HostName nc' +
        'har(40)'#9
      
        'select @UserName=cast(nt_username as nchar(40)), @NetAddress=cas' +
        't(net_address as nchar(40)), @HostName=cast(hostname as nchar(40' +
        ')) from master.dbo.sysprocesses where spid=@@spid'
      ''
      'insert into TDelReception '
      
        '(ReceptionID, CustomerName, CustomerType, ReceptionDate, Recepti' +
        'onType, Shift, ClosedID, StartTime, EndTime, Credit, Active, Mob' +
        'ile, '
      
        'Pool, Saat, AngoshtarPlak, Sooeech, Asnad, Pakhsh, KeefPool, Sam' +
        'sonet, Other, Price, CardID, UserID, Enter, ExitUserID, NumberOf' +
        'Adult, '
      
        'NumberOfChild, CardID2, CardID3, UserName, HostName, NetAddress,' +
        ' DelTime, EnterTime, ReceptionDiscount, OrderNumber, FactorNumbe' +
        'r, '
      
        'AdultPrice, ChildPrice, kindergarten, TicketType, order_id, mobi' +
        'le_number, service_center_id)'
      
        'select ReceptionID, CustomerName, CustomerType, ReceptionDate, R' +
        'eceptionType, Shift, ClosedID, StartTime, EndTime, '
      
        'Credit, Active, Mobile, Pool, Saat, AngoshtarPlak, Sooeech, Asna' +
        'd, Pakhsh, KeefPool, Samsonet, Other, Price, CardID, '
      
        'UserID, Enter, ExitUserID, NumberOfAdult, NumberOfChild, CardID2' +
        ', CardID3, @UserName, @HostName, @NetAddress, '
      
        'dbo.FormatTimeString(cast(Datepart(hh, Getdate()) as char(2))+'#39':' +
        #39'+cast(Datepart(n, Getdate()) as char(2))), '
      
        'EnterTime, ReceptionDiscount, OrderNumber, FactorNumber, AdultPr' +
        'ice, ChildPrice, kindergarten, TicketType, order_id, mobile_numb' +
        'er, service_center_id'
      'from TReception where ReceptionID = @ReceptionID')
    Left = 104
    Top = 368
  end
  object QUpdateReception: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'ReceptionID'
        Attributes = [paSigned]
        DataType = ftLargeint
        Precision = 19
        Size = 8
        Value = Null
      end>
    SQL.Strings = (
      'update TReception set'
      'CardID = 1,'
      'CardID2 = 1,'
      'CardID3 = 1,'
      'price = 0,'
      'NumberOfAdult = 0,'
      'NumberOfChild = 0,'
      'OrderNumber = 0,'
      'ReceptionDiscount = 0,'
      'warmDelete = 1'
      'where ReceptionID = :ReceptionID')
    Left = 144
    Top = 368
  end
end
