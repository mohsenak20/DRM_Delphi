object frmEnrollment: TfrmEnrollment
  Left = 349
  Top = 234
  Width = 1206
  Height = 727
  BiDiMode = bdRightToLeft
  Caption = #1579#1576#1578' '#1606#1575#1605' '#1583#1608#1585#1607
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -16
  Font.Name = 'Koodak'
  Font.Style = []
  KeyPreview = True
  Menu = MainMenuUnit
  OldCreateOrder = False
  ParentBiDiMode = False
  Position = poScreenCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  DesignSize = (
    1190
    668)
  PixelsPerInch = 96
  TextHeight = 28
  object PanelButton: TPanel
    Left = 0
    Top = 619
    Width = 1190
    Height = 49
    Align = alBottom
    BevelInner = bvLowered
    TabOrder = 0
    object BtnNew: TBitBtn
      Left = 1053
      Top = 7
      Width = 87
      Height = 36
      Caption = #1583#1585#1580
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      OnClick = BtnNewClick
      Glyph.Data = {
        
        FF00FFFFFF00FFFFFF00FFFFFF00}
    end
    object BtnEdit: TBitBtn
      Left = 925
      Top = 8
      Width = 87
      Height = 36
      Caption = #1608#1610#1585#1575#1610#1588
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      OnClick = BtnEditClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00}
    end
    object BtnDel: TBitBtn
      Left = 813
      Top = 8
      Width = 87
      Height = 36
      Caption = #1581#1584#1601
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 2
      OnClick = BtnDelClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00D8E9EC00}
    end
    object BtnReturn: TBitBtn
      Left = 26
      Top = 6
      Width = 87
      Height = 36
      Caption = #1576#1575#1586#1711#1588#1578
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 3
      OnClick = BtnReturnClick
      Glyph.Data = {
        }
    end
    object BtnPrint: TBitBtn
      Left = 233
      Top = 7
      Width = 87
      Height = 36
      Caption = #1670#1575#1662' '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 4
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00}
    end
    object btnPay: TBitBtn
      Left = 640
      Top = 8
      Width = 133
      Height = 36
      Caption = #1662#1585#1583#1575#1582#1578' '#1588#1607#1585#1610#1607
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 5
      OnClick = btnPayClick
      Glyph.Data = {
        
        FF00FFFFFF00FFFFFF00FFFFFF00}
    end
  end
  object StatusBar: TStatusBar
    Left = 0
    Top = 600
    Width = 1190
    Height = 19
    Anchors = [akRight, akBottom]
    BiDiMode = bdRightToLeft
    Color = clInfoBk
    Panels = <
      item
        Text = #1576#1575#1586#1711#1588#1578' : Esc'
        Width = 139
      end
      item
        Text = #1670#1575#1662': Ctrl+P'
        Width = 139
      end
      item
        Text = #1581#1584#1601':  Del'
        Width = 139
      end
      item
        Text = #1608#1610#1585#1575#1610#1588':   Ctrl+E'
        Width = 139
      end
      item
        Text = #1583#1585#1580':  Insert'
        Width = 139
      end>
    ParentBiDiMode = False
  end
  object Grid: TDBGrid
    Left = 0
    Top = 130
    Width = 1190
    Height = 470
    TabStop = False
    Align = alClient
    BiDiMode = bdRightToLeft
    DataSource = dsEnrollment
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Traffic'
    Font.Style = [fsBold]
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete]
    ParentBiDiMode = False
    ParentFont = False
    ParentShowHint = False
    PopupMenu = PopupMenuUnit
    ShowHint = False
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    OnDrawColumnCell = GridDrawColumnCell
    OnKeyDown = GridKeyDown
    Columns = <
      item
        Expanded = False
        FieldName = 'id'
        Title.Caption = #1705#1583
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 59
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'name'
        Title.Caption = #1606#1575#1605
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 189
        Visible = True
      end
      item
        Alignment = taRightJustify
        Expanded = False
        FieldName = 'enrollmentDate'
        Title.Alignment = taCenter
        Title.Caption = #1578#1575#1585#1610#1582' '#1579#1576#1578' '#1606#1575#1605
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 77
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'lessonCode'
        Title.Caption = #1705#1583' '#1705#1604#1575#1587
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 61
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'lessonTitle'
        Title.Caption = #1593#1606#1608#1575#1606' '#1705#1604#1575#1587
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 208
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'lessonTypeTitle'
        Title.Caption = #1606#1608#1593' '#1705#1604#1575#1587
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 103
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'lessonGroupTitle'
        Title.Caption = #1711#1585#1608#1607
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 120
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ageCategoryTitle'
        Title.Caption = #1711#1585#1608#1607' '#1587#1606#1610
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 93
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'capacity'
        Title.Caption = #1592#1585#1601#1610#1578
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 54
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'enrollmentPrice'
        Title.Caption = #1602#1610#1605#1578
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 92
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'payment'
        Title.Caption = #1580#1605#1593' '#1662#1585#1583#1575#1582#1578#1610
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 91
        Visible = True
      end>
  end
  object GroupBox: TGroupBox
    Left = 288
    Top = 206
    Width = 898
    Height = 394
    Anchors = [akRight, akBottom]
    Caption = #1605#1588#1582#1589#1575#1578' '
    Color = clBtnFace
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Traffic'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    TabOrder = 3
    Visible = False
    object BtnSave: TBitBtn
      Left = 32
      Top = 40
      Width = 75
      Height = 36
      Caption = #1584#1582#1610#1585#1607
      TabOrder = 0
      OnClick = BtnSaveClick
      Glyph.Data = {
        
        00000000000000000000}
    end
    object BtnCancel: TBitBtn
      Left = 32
      Top = 88
      Width = 75
      Height = 36
      Caption = #1575#1606#1589#1585#1575#1601
      TabOrder = 1
      OnClick = BtnCancelClick
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
    end
    object gbLesson: TGroupBox
      Left = 154
      Top = 32
      Width = 703
      Height = 177
      Caption = #1583#1608#1585#1607
      TabOrder = 2
      object Label20: TLabel
        Left = 615
        Top = 24
        Width = 22
        Height = 29
        Caption = #1705#1583':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label21: TLabel
        Left = 603
        Top = 64
        Width = 42
        Height = 29
        Caption = #1588#1605#1575#1585#1607':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label22: TLabel
        Left = 603
        Top = 100
        Width = 22
        Height = 29
        Caption = #1705#1583':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label23: TLabel
        Left = 400
        Top = 24
        Width = 37
        Height = 29
        Caption = #1593#1606#1608#1575#1606':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label24: TLabel
        Left = 603
        Top = 136
        Width = 66
        Height = 29
        Caption = #1711#1585#1608#1607' '#1587#1606#1610':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label25: TLabel
        Left = 405
        Top = 64
        Width = 24
        Height = 29
        Caption = #1606#1608#1593':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label26: TLabel
        Left = 405
        Top = 100
        Width = 35
        Height = 29
        Caption = #1711#1585#1608#1607':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label27: TLabel
        Left = 405
        Top = 136
        Width = 40
        Height = 29
        Caption = #1602#1610#1605#1578':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label28: TLabel
        Left = 113
        Top = 64
        Width = 52
        Height = 29
        Caption = #1592#1585#1601#1610#1578':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label29: TLabel
        Left = 113
        Top = 100
        Width = 83
        Height = 29
        Caption = #1592#1585#1601#1610#1578' '#1582#1575#1604#1610':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object lblLessonId: TLabel
        Left = 555
        Top = 64
        Width = 42
        Height = 29
        Caption = #1588#1605#1575#1585#1607':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clTeal
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object lblLessonCode: TLabel
        Left = 575
        Top = 100
        Width = 22
        Height = 29
        Caption = #1705#1583':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clTeal
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object lblAgeCategory: TLabel
        Left = 531
        Top = 136
        Width = 66
        Height = 29
        Caption = #1711#1585#1608#1607' '#1587#1606#1610':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clTeal
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object lblLessonType: TLabel
        Left = 373
        Top = 64
        Width = 24
        Height = 29
        Caption = #1606#1608#1593':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clTeal
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object lblLessonGroup: TLabel
        Left = 362
        Top = 100
        Width = 35
        Height = 29
        Caption = #1711#1585#1608#1607':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clTeal
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object lblLessonPrice: TLabel
        Left = 357
        Top = 136
        Width = 40
        Height = 29
        Caption = #1602#1610#1605#1578':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clTeal
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object lblLessonCapacity: TLabel
        Left = 57
        Top = 64
        Width = 52
        Height = 29
        Caption = #1592#1585#1601#1610#1578':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clTeal
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object lblLessonCapacityRemain: TLabel
        Left = 26
        Top = 100
        Width = 83
        Height = 29
        Caption = #1592#1585#1601#1610#1578' '#1582#1575#1604#1610':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clTeal
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object lblLessonTitle: TLabel
        Left = 359
        Top = 25
        Width = 37
        Height = 29
        Caption = #1593#1606#1608#1575#1606':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clTeal
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object btnLessonCode: TBitBtn
        Left = 449
        Top = 24
        Width = 33
        Height = 32
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Koodak'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 1
        OnClick = btnLessonCodeClick
        Glyph.Data = {
          
          EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00}
      end
      object edtLessonCode: TEdit
        Left = 483
        Top = 23
        Width = 127
        Height = 34
        Color = clInfoBk
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Koodak'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 0
      end
    end
    object GroupBox1: TGroupBox
      Left = 152
      Top = 208
      Width = 705
      Height = 169
      Caption = #1588#1582#1589
      TabOrder = 3
      DesignSize = (
        705
        169)
      object Label2: TLabel
        Left = 609
        Top = 42
        Width = 80
        Height = 29
        Caption = #1588#1605#1575#1585#1607' '#1705#1575#1585#1578':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label5: TLabel
        Left = 411
        Top = 42
        Width = 49
        Height = 29
        Caption = #1705#1583' '#1605#1604#1610':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label11: TLabel
        Left = 178
        Top = 42
        Width = 98
        Height = 29
        Caption = #1588#1605#1575#1585#1607' '#1588#1606#1575#1587#1575#1610#1610':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label1: TLabel
        Left = 619
        Top = 132
        Width = 22
        Height = 29
        Caption = #1705#1583':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label3: TLabel
        Left = 392
        Top = 132
        Width = 21
        Height = 29
        Caption = #1606#1575#1605':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object lblCreditCardId: TLabel
        Left = 543
        Top = 132
        Width = 66
        Height = 29
        Caption = #1711#1585#1608#1607' '#1587#1606#1610':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clTeal
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object lblName: TLabel
        Left = 341
        Top = 132
        Width = 40
        Height = 29
        Caption = #1602#1610#1605#1578':'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clTeal
        Font.Height = -16
        Font.Name = 'Traffic'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object edtCreditCardID: TEdit
        Left = 474
        Top = 40
        Width = 127
        Height = 37
        Color = clInfoBk
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Koodak'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 0
      end
      object btnSearchCreditCard: TBitBtn
        Left = 46
        Top = 85
        Width = 643
        Height = 35
        Anchors = [akLeft, akTop, akRight]
        TabOrder = 1
        OnClick = btnSearchCreditCardClick
        Glyph.Data = {
          
          0000}
        NumGlyphs = 2
      end
      object edtNID: TEdit
        Left = 280
        Top = 40
        Width = 127
        Height = 37
        Color = clInfoBk
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Koodak'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 2
      end
      object edtGeneralCode: TEdit
        Left = 48
        Top = 40
        Width = 127
        Height = 37
        Color = clInfoBk
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Koodak'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 3
      end
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1190
    Height = 130
    Align = alTop
    TabOrder = 4
    object Label15: TLabel
      Left = 264
      Top = 63
      Width = 83
      Height = 26
      Caption = #1578#1575#1585#1610#1582' '#1579#1576#1578' '#1606#1575#1605' '#1575#1586':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label16: TLabel
      Left = 120
      Top = 63
      Width = 48
      Height = 26
      Caption = #1575#1604#1610' '#1578#1575#1585#1610#1582':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label4: TLabel
      Left = 401
      Top = 10
      Width = 80
      Height = 29
      Caption = #1588#1605#1575#1585#1607' '#1705#1575#1585#1578':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label6: TLabel
      Left = 659
      Top = 10
      Width = 49
      Height = 29
      Caption = #1705#1583' '#1605#1604#1610':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label7: TLabel
      Left = 162
      Top = 10
      Width = 98
      Height = 29
      Caption = #1588#1605#1575#1585#1607' '#1588#1606#1575#1587#1575#1610#1610':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label8: TLabel
      Left = 1111
      Top = 13
      Width = 22
      Height = 29
      Caption = #1705#1583':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object BBtnAccept: TBitBtn
      Left = 8
      Top = 98
      Width = 1169
      Height = 28
      Caption = #1580#1587#1578#1580#1608
      TabOrder = 0
      OnClick = BBtnAcceptClick
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
    end
    object edtStartDate: TMaskEdit
      Left = 171
      Top = 59
      Width = 92
      Height = 34
      Color = clInfoBk
      EditMask = '1000/00/00;1;_'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      MaxLength = 10
      ParentFont = False
      TabOrder = 1
      Text = '1   /  /  '
    end
    object edtEndDate: TMaskEdit
      Left = 33
      Top = 59
      Width = 88
      Height = 34
      Color = clInfoBk
      EditMask = '1000/00/00;1;_'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      MaxLength = 10
      ParentFont = False
      TabOrder = 2
      Text = '1   /  /  '
    end
    object edtSearchCreditCardCode: TEdit
      Left = 266
      Top = 8
      Width = 127
      Height = 37
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 3
    end
    object edtSearchNID: TEdit
      Left = 496
      Top = 8
      Width = 159
      Height = 37
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 4
    end
    object edtSearchGeneralCode: TEdit
      Left = 32
      Top = 8
      Width = 127
      Height = 37
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 5
    end
    object BitBtn1: TBitBtn
      Left = 745
      Top = 13
      Width = 33
      Height = 32
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 6
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00}
    end
    object edtSearchLessonTitle: TEdit
      Left = 779
      Top = 12
      Width = 267
      Height = 34
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 7
    end
    object edtSearchLessonId: TEdit
      Left = 1047
      Top = 12
      Width = 59
      Height = 34
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 8
    end
    object chbDebtor: TCheckBox
      Left = 1016
      Top = 64
      Width = 97
      Height = 17
      Caption = #1576#1583#1607#1705#1575#1585#1575#1606
      TabOrder = 9
    end
  end
  object MainMenuUnit: TMainMenu
    Left = 72
    Top = 239
    object MnuFile: TMenuItem
      Caption = #1662#1585#1608#1606#1583#1607
      object MnuInsert: TMenuItem
        Caption = #1583#1585#1580
        OnClick = BtnNewClick
      end
      object MnuEdit: TMenuItem
        Caption = #1608#1610#1585#1575#1610#1588
        OnClick = BtnEditClick
      end
      object MnuDelete: TMenuItem
        Caption = #1581#1584#1601
        OnClick = BtnDelClick
      end
      object N2: TMenuItem
        Caption = '-'
      end
      object MnuReturn: TMenuItem
        Caption = #1576#1575#1586#1711#1588#1578
        OnClick = BtnReturnClick
      end
    end
  end
  object PopupMenuUnit: TPopupMenu
    Left = 105
    Top = 239
    object MnuPopInsert: TMenuItem
      Caption = #1583#1585#1580
      OnClick = BtnNewClick
    end
    object MnuPopEdit: TMenuItem
      Caption = #1608#1610#1585#1575#1610#1588
      OnClick = BtnEditClick
    end
    object MnuPopNone1: TMenuItem
      Caption = '-'
    end
    object MnuPopDelete: TMenuItem
      Caption = #1581#1584#1601
      OnClick = BtnDelClick
    end
  end
  object SP_Enrollment: TADOStoredProc
    Connection = DM.ADOConnection
    ProcedureName = 'SP_Enrollment;1'
    Parameters = <
      item
        Name = '@RETURN_VALUE'
        DataType = ftInteger
        Direction = pdReturnValue
        Precision = 10
        Value = Null
      end
      item
        Name = '@id'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@lessonId'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@creditCardId'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@userId'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@Price'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@enrollmentDate'
        Attributes = [paNullable]
        DataType = ftString
        Size = 10
        Value = Null
      end
      item
        Name = '@enrollmentId'
        Attributes = [paNullable]
        DataType = ftInteger
        Direction = pdInputOutput
        Precision = 10
        Value = Null
      end
      item
        Name = '@Action'
        Attributes = [paNullable]
        DataType = ftWideString
        Size = 6
        Value = Null
      end>
    Left = 141
    Top = 239
  end
  object dsEnrollment: TDataSource
    DataSet = QEnrollment
    Left = 176
    Top = 240
  end
  object QEnrollment: TADOQuery
    Connection = DM.ADOConnection
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      
        'select TEnrollment.id id, enrollmentDate, TLesson.id lessonId, T' +
        'Lesson.code '
      
        'lessonCode, TLesson.title lessonTitle, TLessonType.title lessonT' +
        'ypeTitle, '
      
        'TLessonGroup.title lessonGroupTitle, TageCategory.title ageCateg' +
        'oryTitle,'
      
        'TEnrollment.price enrollmentPrice, capacity, TCreditCard.id cred' +
        'itCardId, '
      'TCreditCard.name+'#39' '#39'+TCreditCard.lastName name,'
      
        '(select isnull(sum(price), 0) from TPaymentEnrollment where enro' +
        'llmentId = TEnrollment.id) payment'
      'from TEnrollment '
      
        'inner join TCreditCard on TEnrollment.creditCardId = TCreditCard' +
        '.id'
      'inner join TLesson on TEnrollment.lessonId = TLesson.id'
      'inner join TLessonType on TLesson.lessonTypeId = TLessonType.id'
      
        'inner join TLessonGroup on TLesson.lessonGroupId = TLessonGroup.' +
        'id'
      
        'inner join TageCategory on TLesson.ageCategoryId = TageCategory.' +
        'id')
    Left = 212
    Top = 240
  end
  object ActionList: TActionList
    Left = 72
    Top = 272
    object ActionInsert: TAction
      Caption = 'ActionInsert'
      ShortCut = 45
      OnExecute = BtnNewClick
    end
    object ActionEdit: TAction
      Caption = 'ActionEdit'
      ShortCut = 16453
      OnExecute = BtnEditClick
    end
    object ActionDel: TAction
      Caption = 'ActionDel'
      ShortCut = 46
      OnExecute = BtnDelClick
    end
    object ActionReturn: TAction
      Caption = 'ActionReturn'
      ShortCut = 27
      OnExecute = BtnReturnClick
    end
    object ActionPrint: TAction
      Caption = 'ActionPrint'
      ShortCut = 16464
    end
  end
  object QDelete: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'enrollmentId'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      'delete TEnrollment'
      'where id= :enrollmentId')
    Left = 112
    Top = 280
  end
  object QLesson: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'lessonCode'
        DataType = ftWideString
        NumericScale = 255
        Precision = 255
        Size = 15
        Value = Null
      end>
    SQL.Strings = (
      
        'Select TLesson.*, TLessonType.title lessonTypeTitle, TageCategor' +
        'y.title ageCategoryTitle,'
      'TLessonGroup.title lessonGroupTitle,'
      
        '(select count(id) from TEnrollment where lessonId = TLesson.id) ' +
        'enrollmentCount'
      'From TLesson '
      'inner join TLessonType on TLesson.lessonTypeId = TLessonType.id'
      
        'inner join TageCategory on TLesson.ageCategoryId = TageCategory.' +
        'id'
      
        'inner join TLessonGroup on TLesson.lessonGroupId = TLessonGroup.' +
        'id'
      'where Code = :lessonCode')
    Left = 566
    Top = 264
  end
  object ADOStoredProc1: TADOStoredProc
    Connection = DM.ADOConnection
    ProcedureName = 'SP_Enrollment;1'
    Parameters = <
      item
        Name = '@RETURN_VALUE'
        DataType = ftInteger
        Direction = pdReturnValue
        Precision = 10
        Value = Null
      end
      item
        Name = '@id'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@lessonId'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@creditCardId'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@userId'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@Price'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@enrollmentDate'
        Attributes = [paNullable]
        DataType = ftString
        Size = 10
        Value = Null
      end
      item
        Name = '@enrollmentId'
        Attributes = [paNullable]
        DataType = ftInteger
        Direction = pdInputOutput
        Precision = 10
        Value = Null
      end
      item
        Name = '@Action'
        Attributes = [paNullable]
        DataType = ftWideString
        Size = 6
        Value = Null
      end>
    Left = 638
    Top = 264
  end
  object SP_PaymentEnrollment: TADOStoredProc
    Connection = DM.ADOConnection
    ProcedureName = 'SP_PaymentEnrollment;1'
    Parameters = <
      item
        Name = '@RETURN_VALUE'
        DataType = ftInteger
        Direction = pdReturnValue
        Precision = 10
        Value = Null
      end
      item
        Name = '@id'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@enrollmentId'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@price'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@paymentDate'
        Attributes = [paNullable]
        DataType = ftString
        Size = 10
        Value = Null
      end
      item
        Name = '@userId'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@Action'
        Attributes = [paNullable]
        DataType = ftWideString
        Size = 6
        Value = Null
      end>
    Left = 694
    Top = 264
  end
  object QCreditCard: TADOQuery
    Connection = DM.ADOConnection
    CursorType = ctStatic
    Parameters = <
      item
        Name = 'CreditCardID'
        Size = -1
        Value = Null
      end
      item
        Name = 'NID'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      
        'Declare @purchased int, @used int, @CreditCardID nvarchar(10), @' +
        'NID nvarchar(10), @ChargeDate char(10)'
      'set @CreditCardID = :CreditCardID'
      'set @NID = :NID'
      'if @NID <> '#39'-1'#39
      
        #9'select top 1 @CreditCardID = CardID from TCreditCard where NID ' +
        '= @NID'
      ''
      'if @CreditCardID is null set @CreditCardID = '#39'-1'#39
      'else'
      'begin'
      
        #9'select @purchased = SUM(price) from TCreditCardCharge where Cre' +
        'ditCardID = @CreditCardID'
      ''
      ''
      
        #9'select @used = SUM(AdultPrice*NumberOfAdult) from TReception wh' +
        'ere CreditCardID = @CreditCardID'
      ''
      'end'
      'if @purchased is null set @purchased = 0'
      'if @used is null set @used = 0'
      ''
      ''
      
        'select CardID, Name+'#39' '#39'+lastName Name, NID, FatherName, Address,' +
        ' Tel, CreditGroupID, FreeTicket,'
      
        'Pic, IssueDate, IssueTime, UserID, Active, Type, Title, PriceFor' +
        'Reception, NumberOfCustomer, startFreeTicketDate, endFreeTicketD' +
        'ate,'
      
        'Comment, @used used, @purchased purchased from TCreditCard inner' +
        ' join TCreditCardType on TCreditCard.type = TcreditCardType.id'
      'where CardID = @CreditCardID'
      ''
      '')
    Left = 208
    Top = 328
  end
  object QCheckCanEnrollment: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'lessonId'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end
      item
        Name = 'creditCardId'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      'select * from TEnrollment'
      'where lessonId= :lessonId and creditCardId= :creditCardId')
    Left = 80
    Top = 368
  end
end
