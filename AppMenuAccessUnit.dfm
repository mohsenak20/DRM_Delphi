object frmAppMenuAccess: TfrmAppMenuAccess
  Left = 451
  Top = 129
  Width = 887
  Height = 870
  BiDiMode = bdRightToLeft
  Caption = #1578#1593#1610#1610#1606' '#1583#1587#1578#1585#1587#1610' '#1705#1575#1585#1576#1585#1575#1606' '#1576#1607' '#1605#1606#1608#1607#1575
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -16
  Font.Name = 'Koodak'
  Font.Style = []
  OldCreateOrder = False
  ParentBiDiMode = False
  Position = poDesktopCenter
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 28
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 871
    Height = 89
    Align = alTop
    Anchors = [akTop, akRight]
    TabOrder = 0
    DesignSize = (
      871
      89)
    object Label3: TLabel
      Left = 749
      Top = 36
      Width = 72
      Height = 29
      Anchors = [akTop, akRight]
      Caption = #1576#1582#1588' '#1705#1575#1585#1610':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object edtWorkSectionID: TEdit
      Left = 711
      Top = 28
      Width = 41
      Height = 37
      Anchors = [akTop, akRight]
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
    end
    object edtWorkSectionName: TEdit
      Left = 487
      Top = 28
      Width = 225
      Height = 37
      Anchors = [akTop, akRight]
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
    end
    object btnWorkSection: TBitBtn
      Left = 452
      Top = 29
      Width = 35
      Height = 33
      Anchors = [akTop, akRight]
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 2
      OnClick = btnWorkSectionClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00}
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 790
    Width = 871
    Height = 41
    Align = alBottom
    Anchors = [akTop, akRight]
    TabOrder = 1
  end
  object Panel3: TPanel
    Left = 0
    Top = 89
    Width = 369
    Height = 701
    Align = alLeft
    Anchors = [akTop, akRight]
    TabOrder = 2
    object dbManuAccess: TDBGrid
      Left = 1
      Top = 1
      Width = 367
      Height = 699
      Align = alClient
      DataSource = dsTAppMenuAccess
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
      PopupMenu = pmMenuAccess
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -16
      TitleFont.Name = 'Koodak'
      TitleFont.Style = []
      OnDblClick = BtnDelClick
      Columns = <
        item
          Expanded = False
          FieldName = 'menuId'
          Title.Caption = #1705#1583
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Title'
          Title.Caption = #1593#1606#1608#1575#1606
          Width = 275
          Visible = True
        end>
    end
  end
  object Panel4: TPanel
    Left = 520
    Top = 89
    Width = 351
    Height = 701
    Align = alRight
    Anchors = [akTop, akRight]
    TabOrder = 3
    object dbSource: TDBGrid
      Left = 1
      Top = 1
      Width = 349
      Height = 699
      Align = alClient
      DataSource = dsAppMenu
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
      PopupMenu = pmManu
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -16
      TitleFont.Name = 'Koodak'
      TitleFont.Style = []
      OnDblClick = btnAddClick
      Columns = <
        item
          Expanded = False
          FieldName = 'id'
          Title.Caption = #1705#1583
          Width = 51
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'title'
          Title.Caption = #1593#1606#1608#1575#1606
          Width = 272
          Visible = True
        end>
    end
  end
  object Panel5: TPanel
    Left = 369
    Top = 89
    Width = 151
    Height = 701
    Align = alClient
    Anchors = [akTop, akRight]
    TabOrder = 4
    object BtnDel: TBitBtn
      Left = 32
      Top = 584
      Width = 87
      Height = 36
      Caption = #1581#1584#1601
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      OnClick = BtnDelClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00D8E9EC00}
    end
    object btnAdd: TBitBtn
      Left = 32
      Top = 144
      Width = 87
      Height = 36
      Caption = #1575#1601#1586#1608#1583#1606
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      OnClick = btnAddClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00}
    end
  end
  object dsAppMenu: TDataSource
    DataSet = QAppMenu
    Left = 409
    Top = 121
  end
  object QAppMenu: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <>
    SQL.Strings = (
      'select * from TAppMenu')
    Left = 409
    Top = 169
  end
  object QAppMenuAccess: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'workSectionId'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      
        'select TAppMenuAccess.id id, menuId, Title from TAppMenu inner j' +
        'oin TAppMenuAccess on TAppMenu.id = TAppMenuAccess.menuId'
      'where workSectionId = :workSectionId')
    Left = 417
    Top = 433
  end
  object dsTAppMenuAccess: TDataSource
    DataSet = QAppMenuAccess
    Left = 417
    Top = 489
  end
  object QAdd: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'workSectionId'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end
      item
        Name = 'menuId'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      'insert into TAppMenuAccess (workSectionId, menuId)'
      'values(:workSectionId, :menuId)')
    Left = 430
    Top = 297
  end
  object QDel: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'id'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      'delete TAppMenuAccess'
      'where id = :id')
    Left = 430
    Top = 345
  end
  object pmMenuAccess: TPopupMenu
    Left = 160
    Top = 216
    object pmMenuAccessDelete: TMenuItem
      Caption = #1582#1584#1601
      OnClick = BtnDelClick
    end
  end
  object pmManu: TPopupMenu
    Left = 664
    Top = 225
    object pmMenuAdd: TMenuItem
      Caption = #1575#1601#1586#1608#1583#1606
      OnClick = btnAddClick
    end
  end
end
