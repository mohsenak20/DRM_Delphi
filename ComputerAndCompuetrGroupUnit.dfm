object frmComputerAndCompueterGroup: TfrmComputerAndCompueterGroup
  Left = 473
  Top = 121
  Width = 887
  Height = 869
  BiDiMode = bdRightToLeft
  Caption = #1593#1590#1608#1610#1578' '#1705#1575#1605#1662#1610#1608#1578#1585#1607#1575' '#1583#1585' '#1711#1585#1608#1607#1607#1575
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -16
  Font.Name = 'Koodak'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  ParentBiDiMode = False
  Position = poScreenCenter
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 28
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 871
    Height = 89
    Align = alTop
    Anchors = [akTop, akRight]
    TabOrder = 0
    DesignSize = (
      871
      89)
    object Label3: TLabel
      Left = 755
      Top = 29
      Width = 35
      Height = 29
      Anchors = [akTop, akRight]
      Caption = #1711#1585#1608#1607':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object edtComputerGroupId: TEdit
      Left = 711
      Top = 28
      Width = 41
      Height = 37
      Anchors = [akTop, akRight]
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
    end
    object edtComputerGroupTitle: TEdit
      Left = 487
      Top = 28
      Width = 225
      Height = 37
      Anchors = [akTop, akRight]
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
    end
    object btnComputerGroup: TBitBtn
      Left = 452
      Top = 29
      Width = 35
      Height = 33
      Anchors = [akTop, akRight]
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 2
      OnClick = btnComputerGroupClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00}
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 789
    Width = 871
    Height = 41
    Align = alBottom
    Anchors = [akTop, akRight]
    TabOrder = 1
  end
  object Panel3: TPanel
    Left = 0
    Top = 89
    Width = 369
    Height = 700
    Align = alLeft
    Anchors = [akTop, akRight]
    TabOrder = 2
    object dbManuAccess: TDBGrid
      Left = 1
      Top = 1
      Width = 367
      Height = 698
      Align = alClient
      DataSource = dsComputerAndGroup
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
      PopupMenu = pmMenuAccess
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -16
      TitleFont.Name = 'Koodak'
      TitleFont.Style = []
      OnDblClick = BtnDelClick
      Columns = <
        item
          Expanded = False
          FieldName = 'computerId'
          Title.Caption = #1705#1583
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'computerName'
          Title.Caption = #1593#1606#1608#1575#1606
          Width = 275
          Visible = True
        end>
    end
  end
  object Panel4: TPanel
    Left = 520
    Top = 89
    Width = 351
    Height = 700
    Align = alRight
    Anchors = [akTop, akRight]
    TabOrder = 3
    object dbSource: TDBGrid
      Left = 1
      Top = 1
      Width = 349
      Height = 698
      Align = alClient
      DataSource = dsComputer
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
      PopupMenu = pmManu
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -16
      TitleFont.Name = 'Koodak'
      TitleFont.Style = []
      OnDblClick = btnAddClick
      Columns = <
        item
          Expanded = False
          FieldName = 'ComputerID'
          Title.Caption = #1705#1583
          Width = 51
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ComputerName'
          Title.Caption = #1593#1606#1608#1575#1606
          Width = 272
          Visible = True
        end>
    end
  end
  object Panel5: TPanel
    Left = 369
    Top = 89
    Width = 151
    Height = 700
    Align = alClient
    Anchors = [akTop, akRight]
    TabOrder = 4
    object BtnDel: TBitBtn
      Left = 32
      Top = 584
      Width = 87
      Height = 36
      Caption = #1581#1584#1601
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      OnClick = BtnDelClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00D8E9EC00}
    end
    object btnAdd: TBitBtn
      Left = 32
      Top = 144
      Width = 87
      Height = 36
      Caption = #1575#1601#1586#1608#1583#1606
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      OnClick = btnAddClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00}
    end
  end
  object dsComputer: TDataSource
    DataSet = QComputer
    Left = 409
    Top = 121
  end
  object QComputer: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <>
    SQL.Strings = (
      'select * from TComputer')
    Left = 409
    Top = 169
  end
  object QComputerAndGroup: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'computerGroupId'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      
        'select id, TComputer.* from TComputerAndGroup inner join TComput' +
        'er on TComputerAndGroup.ComuperID = TComputer.ComputerID'
      'where ComputerGroupID = :computerGroupId')
    Left = 417
    Top = 433
  end
  object dsComputerAndGroup: TDataSource
    DataSet = QComputerAndGroup
    Left = 417
    Top = 489
  end
  object QAdd: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'computerGroupId'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end
      item
        Name = 'computerId'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      'insert into TcomputerAndGroup (computerGroupId, ComuperID)'
      'values(:computerGroupId, :computerId)')
    Left = 430
    Top = 297
  end
  object QDel: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'id'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      'delete TComputerAndGroup'
      'where id = :id')
    Left = 430
    Top = 345
  end
  object pmMenuAccess: TPopupMenu
    Left = 160
    Top = 216
    object pmMenuAccessDelete: TMenuItem
      Caption = #1582#1584#1601
    end
  end
  object pmManu: TPopupMenu
    Left = 664
    Top = 225
    object pmMenuAdd: TMenuItem
      Caption = #1575#1601#1586#1608#1583#1606
    end
  end
end
