object frmREceptionCardPrice: TfrmREceptionCardPrice
  Left = 815
  Top = 269
  Width = 338
  Height = 208
  BiDiMode = bdRightToLeft
  Caption = #1583#1585#1575#1605#1583' '#1581#1575#1589#1604' '#1575#1586' '#1662#1584#1610#1585#1588' '#1705#1575#1585#1578' '
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -16
  Font.Name = 'Koodak'
  Font.Style = []
  OldCreateOrder = False
  ParentBiDiMode = False
  Position = poDesktopCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 28
  object Label14: TLabel
    Left = 236
    Top = 15
    Width = 42
    Height = 26
    Caption = #1575#1586' '#1578#1575#1585#1610#1582':'
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Koodak'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label3: TLabel
    Left = 127
    Top = 15
    Width = 15
    Height = 26
    Caption = #1575#1604#1610
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Koodak'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label1: TLabel
    Left = 244
    Top = 79
    Width = 42
    Height = 26
    Caption = #1575#1586' '#1578#1575#1585#1610#1582':'
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Koodak'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object lblPrice: TLabel
    Left = 180
    Top = 79
    Width = 42
    Height = 26
    Caption = #1575#1586' '#1578#1575#1585#1610#1582':'
    Font.Charset = ARABIC_CHARSET
    Font.Color = clRed
    Font.Height = -15
    Font.Name = 'Koodak'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object edtStartDate: TMaskEdit
    Left = 145
    Top = 11
    Width = 90
    Height = 34
    Color = clInfoBk
    EditMask = '1000/00/00;1;_'
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Koodak'
    Font.Style = [fsBold]
    MaxLength = 10
    ParentFont = False
    TabOrder = 0
    Text = '1   /  /  '
  end
  object edtEndDate: TMaskEdit
    Left = 36
    Top = 11
    Width = 88
    Height = 34
    Color = clInfoBk
    EditMask = '1000/00/00;1;_'
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Koodak'
    Font.Style = [fsBold]
    MaxLength = 10
    ParentFont = False
    TabOrder = 1
    Text = '1   /  /  '
  end
  object BtnReturn: TBitBtn
    Left = 10
    Top = 118
    Width = 87
    Height = 36
    Caption = #1576#1575#1586#1711#1588#1578
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Koodak'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 2
    OnClick = BtnReturnClick
    Glyph.Data = {
      }
  end
  object BBtnAccept: TBitBtn
    Left = 105
    Top = 120
    Width = 80
    Height = 34
    Caption = #1580#1587#1578#1580#1608
    TabOrder = 3
    OnClick = BBtnAcceptClick
    Glyph.Data = {
      
      0000}
    NumGlyphs = 2
  end
  object BtnPrint: TBitBtn
    Left = 201
    Top = 119
    Width = 87
    Height = 36
    Caption = #1670#1575#1662' '
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Koodak'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 4
    OnClick = BtnPrintClick
    Glyph.Data = {
      
      EC00D8E9EC00D8E9EC00}
  end
  object QCardPrice: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'StartDate'
        Size = -1
        Value = Null
      end
      item
        Name = 'EndDate'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      
        'Declare @CardPrice1 bigint, @CardPrice2 bigint,'#9'@CardPrice3 bigi' +
        'nt, @ReceptionDate1 char(10), @ReceptionDate2 char(10), @PriceFr' +
        'omFirst bigint, @PriceThisTerm bigint, @price_befor bigint, @ter' +
        'm_price bigint'
      
        'declare @price_used_befor bigint, @price_used_term bigint, @pric' +
        'e_in_term  bigint'
      'set @ReceptionDate1 = :StartDate'
      'set @ReceptionDate2 = :EndDate'
      ''
      
        'select @price_befor = sum(cast(Credit as bigint)) from TCard inn' +
        'er join TBargain on TCard.bargainID = TBargain.BargainID where B' +
        'argainDate < @ReceptionDate1 and GetMoney = 0'
      
        'select @term_price = sum(cast(Credit as bigint)) from TCard inne' +
        'r join TBargain on TCard.bargainID = TBargain.BargainID where Ba' +
        'rgainDate >= @ReceptionDate1 and BargainDate <= @ReceptionDate2 ' +
        'and GetMoney = 0'
      ''
      ''
      
        'select @CardPrice1=SUM(cast(TCard.Credit as bigint)) from TRecep' +
        'tion inner join TCard on TReception.CardID = TCard.CardID'
      'inner join TBargain on TCard.BargainID = TBargain.BargainID'
      
        'where ReceptionDate < @ReceptionDate1 and TReception.CardID > 4 ' +
        'and GetMoney = 0'
      ''
      ''
      
        'select @CardPrice2=SUM(cast(TCard.Credit as bigint)) from TRecep' +
        'tion inner join TCard on TReception.CardID2 = TCard.CardID'
      'inner join TBargain on TCard.BargainID = TBargain.BargainID'
      
        'where ReceptionDate < @ReceptionDate1 and TReception.CardID > 4 ' +
        'and GetMoney = 0'
      ''
      ''
      
        'select @CardPrice3=SUM(cast(TCard.Credit as bigint)) from TRecep' +
        'tion inner join TCard on TReception.CardID3 = TCard.CardID'
      'inner join TBargain on TCard.BargainID = TBargain.BargainID'
      
        'where ReceptionDate < @ReceptionDate1 and TReception.CardID > 4 ' +
        'and GetMoney = 0'
      ''
      ''
      'if @CardPrice1 is null'
      #9'set @CardPrice1 = 0'
      ''
      'if @CardPrice2 is null'
      #9'set @CardPrice2 = 0'
      ''
      'if @CardPrice3 is null'
      #9'set @CardPrice3 = 0'
      ''
      'set @price_used_befor = @CardPrice1+@CardPrice2+@CardPrice3'
      ''
      ''
      ''
      ''
      
        'select @CardPrice1=SUM(cast(TCard.Credit as bigint)) from TRecep' +
        'tion inner join TCard on TReception.CardID = TCard.CardID'
      'inner join TBargain on TCard.BargainID = TBargain.BargainID'
      
        'where ReceptionDate >= @ReceptionDate1 and ReceptionDate <= @Rec' +
        'eptionDate2 and TReception.CardID > 4 and GetMoney = 0'
      ''
      ''
      
        'select @CardPrice2=SUM(cast(TCard.Credit as bigint)) from TRecep' +
        'tion inner join TCard on TReception.CardID2 = TCard.CardID'
      'inner join TBargain on TCard.BargainID = TBargain.BargainID'
      
        'where ReceptionDate >= @ReceptionDate1 and ReceptionDate <= @Rec' +
        'eptionDate2 and TReception.CardID > 4 and GetMoney = 0'
      ''
      ''
      
        'select @CardPrice3=SUM(cast(TCard.Credit as bigint)) from TRecep' +
        'tion inner join TCard on TReception.CardID3 = TCard.CardID'
      'inner join TBargain on TCard.BargainID = TBargain.BargainID'
      
        'where ReceptionDate >= @ReceptionDate1 and ReceptionDate <= @Rec' +
        'eptionDate2 and TReception.CardID > 4 and GetMoney = 0'
      ''
      ''
      'if @CardPrice1 is null'
      #9'set @CardPrice1 = 0'
      ''
      'if @CardPrice2 is null'
      #9'set @CardPrice2 = 0'
      ''
      'if @CardPrice3 is null'
      #9'set @CardPrice3 = 0'
      ''
      'set @price_used_term = @CardPrice1+@CardPrice2+@CardPrice3'
      ''
      ''
      
        'select @CardPrice1=SUM(cast(TCard.Credit as bigint)) from TRecep' +
        'tion inner join TCard on TReception.CardID = TCard.CardID'
      'inner join TBargain on TCard.BargainID = TBargain.BargainID'
      
        'where ReceptionDate >= @ReceptionDate1 and ReceptionDate <= @Rec' +
        'eptionDate2 and TReception.CardID > 4 and GetMoney = 0 and Barga' +
        'inDate >= @ReceptionDate1 and BargainDate <= @ReceptionDate2'
      ''
      ''
      
        'select @CardPrice2=SUM(cast(TCard.Credit as bigint)) from TRecep' +
        'tion inner join TCard on TReception.CardID2 = TCard.CardID'
      'inner join TBargain on TCard.BargainID = TBargain.BargainID'
      
        'where ReceptionDate >= @ReceptionDate1 and ReceptionDate <= @Rec' +
        'eptionDate2 and TReception.CardID > 4 and GetMoney = 0 and Barga' +
        'inDate >= @ReceptionDate1 and BargainDate <= @ReceptionDate2'
      ''
      ''
      
        'select @CardPrice3=SUM(cast(TCard.Credit as bigint)) from TRecep' +
        'tion inner join TCard on TReception.CardID3 = TCard.CardID'
      'inner join TBargain on TCard.BargainID = TBargain.BargainID'
      
        'where ReceptionDate >= @ReceptionDate1 and ReceptionDate <= @Rec' +
        'eptionDate2 and TReception.CardID > 4 and GetMoney = 0 and Barga' +
        'inDate >= @ReceptionDate1 and BargainDate <= @ReceptionDate2'
      ''
      ''
      'if @CardPrice1 is null'
      #9'set @CardPrice1 = 0'
      ''
      'if @CardPrice2 is null'
      #9'set @CardPrice2 = 0'
      ''
      'if @CardPrice3 is null'
      #9'set @CardPrice3 = 0'
      ''
      'set @price_in_term = @CardPrice1+@CardPrice2+@CardPrice3'
      ''
      ''
      ''
      
        'select @price_befor as price_befor, @term_price as term_price, @' +
        'price_used_befor as price_used_befor, @price_in_term as price_in' +
        '_term, '
      
        '@price_used_term as price_used_term, @price_befor+@term_price-@p' +
        'rice_used_befor-@price_used_term as price_mod')
    Left = 24
    Top = 64
  end
end
