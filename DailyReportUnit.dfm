object frmDailyReport: TfrmDailyReport
  Left = 281
  Top = 200
  Width = 869
  Height = 640
  BiDiMode = bdRightToLeft
  Caption = #1711#1586#1575#1585#1588' '#1585#1608#1586#1575#1606#1607
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -16
  Font.Name = 'Koodak'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  ParentBiDiMode = False
  Position = poDesktopCenter
  OnKeyDown = FormKeyDown
  PixelsPerInch = 96
  TextHeight = 28
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 853
    Height = 97
    Align = alTop
    TabOrder = 0
    object Label14: TLabel
      Left = 802
      Top = 15
      Width = 42
      Height = 26
      Caption = #1575#1586' '#1578#1575#1585#1610#1582':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label1: TLabel
      Left = 694
      Top = 15
      Width = 15
      Height = 26
      Caption = #1575#1604#1610
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object edtStartDate: TMaskEdit
      Left = 713
      Top = 11
      Width = 88
      Height = 34
      Color = clInfoBk
      EditMask = '1000/00/00;1;_'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      MaxLength = 10
      ParentFont = False
      TabOrder = 0
      Text = '1   /  /  '
    end
    object BBtnAccept: TBitBtn
      Left = 8
      Top = 57
      Width = 841
      Height = 24
      Caption = #1580#1587#1578#1580#1608
      TabOrder = 2
      OnClick = BBtnAcceptClick
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
    end
    object edtEndDate: TMaskEdit
      Left = 605
      Top = 11
      Width = 88
      Height = 34
      Color = clInfoBk
      EditMask = '1000/00/00;1;_'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      MaxLength = 10
      ParentFont = False
      TabOrder = 1
      Text = '1   /  /  '
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 480
    Width = 853
    Height = 121
    Align = alBottom
    TabOrder = 1
    object BtnReturn: TBitBtn
      Left = 10
      Top = 77
      Width = 87
      Height = 36
      Caption = #1576#1575#1586#1711#1588#1578
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      OnClick = BtnReturnClick
      Glyph.Data = {
        }
    end
    object BtnPrint: TBitBtn
      Left = 104
      Top = 78
      Width = 87
      Height = 36
      Caption = #1670#1575#1662' '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      OnClick = BtnPrintClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00}
    end
    object BitBtn1: TBitBtn
      Left = 200
      Top = 78
      Width = 161
      Height = 36
      Caption = #1585#1587#1578#1608#1585#1575#1606' '#1608' '#1705#1575#1601#1610' '#1588#1575#1662
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 2
      Visible = False
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00}
    end
    object btnShop: TBitBtn
      Left = 368
      Top = 77
      Width = 161
      Height = 36
      Caption = #1601#1585#1608#1588#1711#1575#1607#1607#1575
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 3
      OnClick = btnShopClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00}
    end
    object ExportToExcel: TBitBtn
      Left = 709
      Top = 79
      Width = 124
      Height = 36
      Caption = #1575#1585#1587#1575#1604' '#1576#1607' '#1575#1705#1587#1604
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 4
      OnClick = ExportToExcelClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00}
    end
    object BitBtn2: TBitBtn
      Left = 536
      Top = 77
      Width = 161
      Height = 36
      Caption = #1601#1585#1608#1588#1711#1575#1607#1607#1575' - '#1575#1705#1587#1604
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 5
      OnClick = BitBtn2Click
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00}
    end
    object BitBtn3: TBitBtn
      Left = 368
      Top = 29
      Width = 161
      Height = 36
      Caption = #1601#1585#1608#1588#1711#1575#1607#1607#1575'2'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 6
      OnClick = BitBtn3Click
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00}
    end
  end
  object Grid: TDBGrid
    Left = 0
    Top = 97
    Width = 853
    Height = 383
    TabStop = False
    Align = alClient
    BiDiMode = bdRightToLeft
    DataSource = dsDailyReport
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Koodak'
    Font.Style = [fsBold]
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete]
    ParentBiDiMode = False
    ParentFont = False
    ParentShowHint = False
    ShowHint = False
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'ReceptionDate'
        Title.Caption = #1578#1575#1585#1610#1582
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'koodak'
        Title.Font.Style = []
        Width = 112
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Reception'
        Title.Caption = #1662#1584#1610#1585#1588
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 125
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'TBK'
        Title.Caption = #1605#1580#1605#1608#1593
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 73
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Card'
        Title.Caption = #1578#1593#1583#1575#1583' '#1705#1575#1585#1578
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'koodak'
        Title.Font.Style = []
        Width = 70
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Amrieh'
        Title.Caption = #1575#1605#1585#1610#1607
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 81
        Visible = True
      end>
  end
  object QDailyReport: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'StartDate'
        Size = -1
        Value = Null
      end
      item
        Name = 'EndDate'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      ''
      #9'declare @StartDate'#9'Char(10), @EndDate'#9'Char(50)'
      #9'set @StartDate = :StartDate'
      #9'set @EndDate = :EndDate'
      ''
      
        'select ReceptionDate, SUM(NumberOfAdult) TedadB, SUM(NumberOfChi' +
        'ld) TedadK, SUM(NumberOfAdult+NumberOfChild) TBK,'
      
        '(select count(ReceptionID) from TReception where ReceptionDate =' +
        ' TR.ReceptionDate and CardID > 4)+'
      
        '(select count(ReceptionID) from TReception where ReceptionDate =' +
        ' TR.ReceptionDate and CardID2 > 4)+'
      
        '(select count(ReceptionID) from TReception where ReceptionDate =' +
        ' TR.ReceptionDate and CardID3 > 4) Card,'
      
        '(select SUM(NumberOfAdult) from TReception where ReceptionDate =' +
        ' TR.ReceptionDate and OrderNumber > 0) Amrieh,'
      
        '(select Sum(cast(Price as bigint)) from TReception where Recepti' +
        'onDate=TR.ReceptionDate) as Reception'
      'from TReception as TR'
      'where ReceptionDate >= @StartDate and ReceptionDate <= @EndDate'
      'group by ReceptionDate'
      'order by ReceptionDate')
    Left = 56
    Top = 16
  end
  object dsDailyReport: TDataSource
    DataSet = QDailyReport
    Left = 96
    Top = 16
  end
  object SP_DailyReport: TADOStoredProc
    Connection = DM.ADOConnection
    ProcedureName = 'SP_DailyReport;1'
    Parameters = <
      item
        Name = '@RETURN_VALUE'
        DataType = ftInteger
        Direction = pdReturnValue
        Precision = 10
        Value = Null
      end
      item
        Name = '@StartDate'
        Attributes = [paNullable]
        DataType = ftString
        Size = 10
        Value = Null
      end
      item
        Name = '@EndDate'
        Attributes = [paNullable]
        DataType = ftString
        Size = 50
        Value = Null
      end>
    Left = 144
    Top = 16
  end
  object QSumResturent: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'StartDate'
        Size = -1
        Value = Null
      end
      item
        Name = 'EndDate'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'Declare @StartDate char(10)'
      'Declare @EndDate char(10)'
      'set @StartDate= :StartDate'
      'set @EndDate= :EndDate'
      'select sum(Number*VahedPrice) as SumResturent,'
      
        '(select sum(Number*vahedPrice) from TReception inner join TFacto' +
        'r on TReception.ReceptionID=TFactor.ReceptionID inner join TSubF' +
        'actor on  TFactor.FactorID=TSubFactor.FactorID where TFactor.Use' +
        'rID=20  and ReceptionDate >= @StartDate and ReceptionDate <= @En' +
        'dDate) as SumCofeeShop,'
      
        '(select sum(Number*vahedPrice) from TReception inner join TFacto' +
        'r on TReception.ReceptionID=TFactor.ReceptionID inner join TSubF' +
        'actor on  TFactor.FactorID=TSubFactor.FactorID where (TFactor.Us' +
        'erID=20  or  TFactor.UserID=4 or  TFactor.UserID=32 or  TFactor.' +
        'UserID=38)and ReceptionDate >= @StartDate and ReceptionDate <= @' +
        'EndDate and ProductID<>157) as SumResturentCofeeShop,'
      
        '(select sum(Number*vahedPrice) from TReception inner join TFacto' +
        'r on TReception.ReceptionID=TFactor.ReceptionID inner join TSubF' +
        'actor on TFactor.FactorID=TSubFactor.FactorID where TFactor.User' +
        'ID=28  and ReceptionDate >= @StartDate and ReceptionDate <= @End' +
        'Date) as SumPhoto,'
      
        '(select sum(Number*vahedPrice) from TReception inner join TFacto' +
        'r on TReception.ReceptionID=TFactor.ReceptionID inner join TSubF' +
        'actor on TFactor.FactorID=TSubFactor.FactorID where TFactor.User' +
        'ID=38  and ReceptionDate >= @StartDate and ReceptionDate <= @End' +
        'Date) as SumRevagh,'
      
        '(select sum(Number*vahedPrice) from TReception inner join TFacto' +
        'r on TReception.ReceptionID=TFactor.ReceptionID inner join TSubF' +
        'actor on TFactor.FactorID=TSubFactor.FactorID where TFactor.User' +
        'ID=32  and ReceptionDate >= @StartDate and ReceptionDate <= @End' +
        'Date and ProductID<>157) as SumSona'
      
        'from TReception inner join TFactor on TReception.ReceptionID=TFa' +
        'ctor.ReceptionID inner join TSubFactor on TFactor.FactorID=TSubF' +
        'actor.FactorID'
      
        'where TFactor.UserID=4 and ReceptionDate >= @StartDate and Recep' +
        'tionDate <= @EndDate')
    Left = 160
    Top = 501
  end
  object SP_Shop: TADOStoredProc
    Connection = DM.ADOConnection
    ProcedureName = 'SP_ShopReport;1'
    Parameters = <
      item
        Name = '@RETURN_VALUE'
        DataType = ftInteger
        Direction = pdReturnValue
        Precision = 10
        Value = Null
      end
      item
        Name = '@StartDate'
        Attributes = [paNullable]
        DataType = ftString
        Size = 10
        Value = Null
      end
      item
        Name = '@EndDate'
        Attributes = [paNullable]
        DataType = ftString
        Size = 50
        Value = Null
      end>
    Left = 184
    Top = 16
  end
  object QShopReport: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'StartDate'
        Size = -1
        Value = Null
      end
      item
        Name = 'EndDate'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'Declare @temp table'
      '('
      #9'ReceptionDate char(10),'
      #9'UserID'#9#9#9'int,'
      #9'sumFactor'#9#9'bigint not null default 0'
      ')'
      ''
      'Declare @StartDate char(10), @EndDate Char(10)'
      'set @StartDate= :StartDate'
      'set @EndDate= :EndDate'
      'insert into @temp'
      #9'(Receptiondate, UserID, sumFactor)'
      
        '(Select ReceptionDate, TFactor.UserID, Sum(Number*TSubFactor.Vah' +
        'edPrice) from'
      
        'TReception inner join TFactor on TReception.ReceptionID=TFactor.' +
        'ReceptionID'
      'inner join TSubFactor on TFactor.FactorID=TSubFactor.FactorID'
      'where ReceptionDate>= @StartDate and ReceptionDate<= @EndDate'
      'group by ReceptionDate, TFactor.UserID)'
      'order by ReceptionDate, TFactor.UserID'
      ''
      ''
      ''
      ''
      '--insert into @temp'
      #9'--(Receptiondate, UserID, sumFactor)'
      
        '--(Select ReceptionDate, ProductID, Sum(Number*TSubFactor.VahedP' +
        'rice) from TReception'
      
        '--inner join TFactor on TReception.ReceptionID=TFactor.Reception' +
        'ID'
      '--inner join TSubFactor on TFactor.FactorID=TSubFactor.FactorID'
      
        '--where ReceptionDate>= @StartDate and ReceptionDate<= @EndDate ' +
        'and (ProductID=330 or ProductID=331 or ProductID=157 or ProductI' +
        'D=332)'
      '--Group by ReceptionDate, ProductID)'
      '--order by ReceptionDate, productID'
      ''
      '--select * from @sonatemp'
      '--select * from @Temp'
      ''
      
        'select ReceptionDate, isnull((select sum(sumFactor) from @Temp w' +
        'here ReceptionDate=T.ReceptionDate), 0) as sum,'
      
        'isnull((select sumFactor from @Temp where UserID=4 and Reception' +
        'Date=T.ReceptionDate), 0) as Fastfood,'
      
        'isnull((select sumFactor from @Temp where UserID=377 and Recepti' +
        'onDate=T.ReceptionDate), 0) as Fastfood2,'
      
        'isnull((select sumFactor from @Temp where UserID=378 and Recepti' +
        'onDate=T.ReceptionDate), 0) as CoffeeShop2,'
      
        'isnull((select sumFactor from @Temp where UserID=49 and Receptio' +
        'nDate=T.ReceptionDate), 0) as sport1, '
      
        'isnull((select sumFactor from @Temp where UserID=50 and Receptio' +
        'nDate=T.ReceptionDate), 0)as Sport2,'
      
        'isnull((select sumFactor from @Temp where UserID=281 and Recepti' +
        'onDate=T.ReceptionDate), 0) as HamamTorkeyBanovan,'
      
        'isnull((select sumFactor from @Temp where UserID=285 and Recepti' +
        'onDate=T.ReceptionDate), 0) as HamamTorkeyAgayan,'
      
        'isnull((select sumFactor from @Temp where UserID=284 and Recepti' +
        'onDate=T.ReceptionDate), 0) as HairStyler,'
      
        'isnull((select sumFactor from @Temp where UserID=283 and Recepti' +
        'onDate=T.ReceptionDate), 0) as BarberShop,'
      ''
      
        'isnull((select sumFactor from @Temp where UserID=28 and Receptio' +
        'nDate=T.ReceptionDate), 0) as Photo,'
      
        'isnull((select sumFactor from @Temp where UserID=186 and Recepti' +
        'onDate=T.ReceptionDate), 0) as TornadoSlide,'
      
        'isnull((select sumFactor from @Temp where UserID=52 and Receptio' +
        'nDate=T.ReceptionDate), 0) as Orange,'
      
        'isnull((select sumFactor from @Temp where UserID=95 and Receptio' +
        'nDate=T.ReceptionDate), 0) as Joice,'
      
        'isnull((select sumFactor from @Temp where UserID=38 and Receptio' +
        'nDate=T.ReceptionDate), 0) as Revagh,'
      ''
      
        'isnull((select sumFactor from @Temp where UserID=32 and Receptio' +
        'nDate=T.ReceptionDate), 0) as sona,'
      
        'isnull((select sumFactor from @Temp where UserID=196 and Recepti' +
        'onDate=T.ReceptionDate), 0) as JetMoj,'
      
        'isnull((select sumFactor from @Temp where UserID=312 and Recepti' +
        'onDate=T.ReceptionDate), 0) as Moje,'
      
        'isnull((select sumFactor from @Temp where UserID=184 and Recepti' +
        'onDate=T.ReceptionDate), 0) as Loop,'
      
        'isnull((select sumFactor from @Temp where UserID=194 and Recepti' +
        'onDate=T.ReceptionDate), 0) as Fazayee'
      ''
      ''
      'from @Temp as T'
      'where UserID=4'
      '')
    Left = 200
    Top = 501
  end
  object QSumSpecial: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'StartDate'
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 10
        Value = Null
      end
      item
        Name = 'EndDate'
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 10
        Value = Null
      end>
    SQL.Strings = (
      
        'select sum(NumberOfAdult)+sum(NumberOfChild) Kol, sum(NumberOfAd' +
        'ult) b, sum(NumberOfChild) k,'
      
        'cast ((sum(NumberOfAdult)+sum(NumberOfChild))*68/100 as int) car' +
        'd from TReception'
      
        'where ReceptionDate>= :StartDate and ReceptionDate<= :EndDate an' +
        'd ReceptionDate not between '#39'1386/10/17'#39' and '#39'1386/11/17'#39
      ' and TReception.ReceptionID%2=0')
    Left = 240
    Top = 504
  end
  object QSumPrice: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'StartDate'
        Size = -1
        Value = Null
      end
      item
        Name = 'EndDate'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'Declare @StartDate char(10), @EndDate Char(10)'
      'set @StartDate= :StartDate'
      'set @EndDate= :EndDate'
      ''
      'select SUM(cast(VahedPrice as bigint)*Number) as SumFastFood,'
      
        '(select SUM(cast(VahedPrice as bigint)*Number) from TReception i' +
        'nner join TFactor  on TReception.ReceptionID = TFactor.Reception' +
        'ID'
      'inner join TSubFactor on TFactor.FactorID = TSubFactor.FactorID'
      
        'where ReceptionDate >=@StartDate and ReceptionDate <= @EndDate) ' +
        'as sumFactor,'
      
        '(select SUM(cast(VahedPrice as bigint)*Number) from TReception i' +
        'nner join TFactor  on TReception.ReceptionID = TFactor.Reception' +
        'ID'
      'inner join TSubFactor on TFactor.FactorID = TSubFactor.FactorID'
      
        'where ReceptionDate >=@StartDate and ReceptionDate <= @EndDate  ' +
        'and (TFactor.UserID = 49 or TFactor.UserID = 50) ) as sumSport,'
      ''
      
        '(select SUM(cast(VahedPrice as bigint)*Number) from TReception i' +
        'nner join TFactor  on TReception.ReceptionID = TFactor.Reception' +
        'ID'
      'inner join TSubFactor on TFactor.FactorID = TSubFactor.FactorID'
      
        'where ReceptionDate >=@StartDate and ReceptionDate <= @EndDate  ' +
        'and TFactor.UserID = 49 ) as sumSport1,'
      ''
      
        '(select SUM(cast(VahedPrice as bigint)*Number) from TReception i' +
        'nner join TFactor  on TReception.ReceptionID = TFactor.Reception' +
        'ID'
      'inner join TSubFactor on TFactor.FactorID = TSubFactor.FactorID'
      
        'where ReceptionDate >=@StartDate and ReceptionDate <= @EndDate  ' +
        'and TFactor.UserID = 50 ) as sumSport2,'
      
        '(select SUM(cast(VahedPrice as bigint)*Number) from TReception i' +
        'nner join TFactor  on TReception.ReceptionID = TFactor.Reception' +
        'ID'
      'inner join TSubFactor on TFactor.FactorID = TSubFactor.FactorID'
      
        'where ReceptionDate >=@StartDate and ReceptionDate <= @EndDate  ' +
        'and TFactor.UserID = 38) as sumRevagh,'
      
        '(select SUM(cast(VahedPrice as bigint)*Number) from TReception i' +
        'nner join TFactor  on TReception.ReceptionID = TFactor.Reception' +
        'ID'
      'inner join TSubFactor on TFactor.FactorID = TSubFactor.FactorID'
      
        'where ReceptionDate >=@StartDate and ReceptionDate <= @EndDate  ' +
        'and TFactor.UserID = 378 ) as sumCoffeeShop2,'
      
        '(select SUM(cast(VahedPrice as bigint)*Number) from TReception i' +
        'nner join TFactor  on TReception.ReceptionID = TFactor.Reception' +
        'ID'
      'inner join TSubFactor on TFactor.FactorID = TSubFactor.FactorID'
      
        'where ReceptionDate >=@StartDate and ReceptionDate <= @EndDate  ' +
        'and TFactor.UserID = 377) as sumFastFood2,'
      ''
      
        '(select SUM(cast(VahedPrice as bigint)*Number) from TReception i' +
        'nner join TFactor  on TReception.ReceptionID = TFactor.Reception' +
        'ID'
      'inner join TSubFactor on TFactor.FactorID = TSubFactor.FactorID'
      
        'where ReceptionDate >=@StartDate and ReceptionDate <= @EndDate  ' +
        'and TFactor.UserID = 32) as sumSona,'
      ''
      
        '(select SUM(cast(VahedPrice as bigint)*Number) from TReception i' +
        'nner join TFactor  on TReception.ReceptionID = TFactor.Reception' +
        'ID'
      'inner join TSubFactor on TFactor.FactorID = TSubFactor.FactorID'
      
        'where ReceptionDate >=@StartDate and ReceptionDate <= @EndDate  ' +
        'and TFactor.UserID = 28) as sumPhoto,'
      ''
      
        '(select SUM(cast(VahedPrice as bigint)*Number) from TReception i' +
        'nner join TFactor  on TReception.ReceptionID = TFactor.Reception' +
        'ID'
      'inner join TSubFactor on TFactor.FactorID = TSubFactor.FactorID'
      
        'where ReceptionDate >=@StartDate and ReceptionDate <= @EndDate  ' +
        'and TFactor.UserID = 52) as sumOrange,'
      ''
      
        '(select SUM(cast(VahedPrice as bigint)*Number) from TReception i' +
        'nner join TFactor  on TReception.ReceptionID = TFactor.Reception' +
        'ID'
      'inner join TSubFactor on TFactor.FactorID = TSubFactor.FactorID'
      
        'where ReceptionDate >=@StartDate and ReceptionDate <= @EndDate  ' +
        'and TFactor.UserID = 285) as hamamTorkiAghayan,'
      ''
      
        '(select SUM(cast(VahedPrice as bigint)*Number) from TReception i' +
        'nner join TFactor  on TReception.ReceptionID = TFactor.Reception' +
        'ID'
      'inner join TSubFactor on TFactor.FactorID = TSubFactor.FactorID'
      
        'where ReceptionDate >=@StartDate and ReceptionDate <= @EndDate  ' +
        'and TFactor.UserID = 281) as hamamTorkiBanovan'
      ''
      
        ' from TReception inner join TFactor  on TReception.ReceptionID =' +
        ' TFactor.ReceptionID'
      'inner join TSubFactor on TFactor.FactorID = TSubFactor.FactorID'
      
        'where ReceptionDate >=@StartDate and ReceptionDate <= @EndDate a' +
        'nd TFactor.UserID = 4')
    Left = 288
    Top = 501
  end
  object QShop2: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'StartDate'
        Size = -1
        Value = Null
      end
      item
        Name = 'EndDate'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      
        'select TFactor.UserID, TUser.UserName ShopName, sum(cast(Number ' +
        'as bigint)*VahedPrice) AmareForoosh'
      
        'from TReception inner join TFactor on TReception.ReceptionID = T' +
        'Factor.ReceptionID'
      'inner join TSubFactor on TFactor.FactorID = TSubFactor.FactorID'
      'inner join TUser on TFactor.UserID = TUser.UserID'
      'where ReceptionDate >= :StartDate and ReceptionDate <= :EndDate '
      'group by TFactor.UserID, TUser.UserName')
    Left = 560
    Top = 496
  end
end
