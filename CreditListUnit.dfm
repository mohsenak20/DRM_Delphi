object frmCreditList: TfrmCreditList
  Left = 136
  Top = 153
  Width = 895
  Height = 582
  BiDiMode = bdRightToLeft
  Caption = #1604#1610#1587#1578' '#1588#1575#1585#1688
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -16
  Font.Name = 'Koodak'
  Font.Style = []
  KeyPreview = True
  Menu = MainMenuUnit
  OldCreateOrder = False
  ParentBiDiMode = False
  Position = poDesktopCenter
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 28
  object PanelButton: TPanel
    Left = 0
    Top = 476
    Width = 879
    Height = 47
    Align = alBottom
    BevelInner = bvLowered
    TabOrder = 0
    DesignSize = (
      879
      47)
    object Label7: TLabel
      Left = 490
      Top = 10
      Width = 38
      Height = 29
      Caption = #1578#1593#1583#1575#1583':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
      Visible = False
    end
    object lblTedad: TLabel
      Left = 391
      Top = 11
      Width = 96
      Height = 29
      Caption = #1578#1593#1583#1575#1583' '#1576#1586#1585#1711#1587#1575#1604':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clMaroon
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
      Visible = False
    end
    object Label10: TLabel
      Left = 305
      Top = 10
      Width = 61
      Height = 29
      Caption = #1580#1605#1593' '#1588#1575#1585#1688':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
      Visible = False
    end
    object lblSum: TLabel
      Left = 202
      Top = 10
      Width = 96
      Height = 29
      Caption = #1578#1593#1583#1575#1583' '#1576#1586#1585#1711#1587#1575#1604':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clMaroon
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
      Visible = False
    end
    object BtnDel: TBitBtn
      Left = 785
      Top = 8
      Width = 87
      Height = 36
      Anchors = [akTop, akRight]
      Caption = #1581#1584#1601
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      OnClick = BtnDelClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00D8E9EC00}
    end
    object BtnReturn: TBitBtn
      Left = 18
      Top = 6
      Width = 87
      Height = 36
      Caption = #1576#1575#1586#1711#1588#1578
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      Glyph.Data = {
        }
    end
    object BtnPrint: TBitBtn
      Left = 669
      Top = 7
      Width = 87
      Height = 36
      Anchors = [akTop, akRight]
      Caption = #1670#1575#1662' '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 2
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00}
    end
  end
  object StatusBar: TStatusBar
    Left = 0
    Top = 457
    Width = 879
    Height = 19
    Anchors = [akRight, akBottom]
    BiDiMode = bdRightToLeft
    Color = clInfoBk
    Panels = <
      item
        Text = #1576#1575#1586#1711#1588#1578' : Esc'
        Width = 139
      end
      item
        Text = #1670#1575#1662': Ctrl+P'
        Width = 139
      end
      item
        Text = #1581#1584#1601':  Del'
        Width = 139
      end
      item
        Text = #1608#1610#1585#1575#1610#1588':   Ctrl+E'
        Width = 139
      end
      item
        Text = #1583#1585#1580':  Insert'
        Width = 139
      end>
    ParentBiDiMode = False
  end
  object Grid: TDBGrid
    Left = 0
    Top = 121
    Width = 879
    Height = 336
    TabStop = False
    Align = alClient
    BiDiMode = bdRightToLeft
    DataSource = dsCredit
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Traffic'
    Font.Style = [fsBold]
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete]
    ParentBiDiMode = False
    ParentFont = False
    ParentShowHint = False
    PopupMenu = PopupMenuUnit
    ShowHint = False
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    OnKeyDown = FormKeyDown
    OnMouseMove = GridMouseMove
    OnTitleClick = GridTitleClick
    Columns = <
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'CreditID'
        Title.Alignment = taCenter
        Title.Caption = #1705#1583' '#1575#1593#1578#1576#1575#1585
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'ReceptionID'
        Title.Alignment = taCenter
        Title.Caption = #1705#1583' '#1662#1584#1610#1585#1588
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 118
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'Credit'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Koodak'
        Font.Style = [fsBold]
        Title.Alignment = taCenter
        Title.Caption = #1575#1593#1578#1576#1575#1585
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 83
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'ChargeTime'
        Title.Alignment = taCenter
        Title.Caption = #1586#1605#1575#1606' '#1588#1575#1585#1688
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CustomerName'
        Title.Alignment = taCenter
        Title.Caption = #1606#1575#1605' '#1605#1588#1578#1585#1610
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 169
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'StartTime'
        Title.Alignment = taCenter
        Title.Caption = #1587#1575#1593#1578' '#1588#1585#1608#1593
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 91
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'Keynumber'
        Title.Alignment = taCenter
        Title.Caption = #1588#1605#1575#1585#1607' '#1705#1605#1583
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 66
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'strPositionClosed'
        Title.Alignment = taCenter
        Title.Caption = #1605#1705#1575#1606' '#1705#1605#1583
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Visible = True
      end>
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 879
    Height = 121
    Align = alTop
    TabOrder = 3
    object Label4: TLabel
      Left = 776
      Top = 11
      Width = 21
      Height = 29
      Caption = #1606#1575#1605':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label5: TLabel
      Left = 775
      Top = 51
      Width = 94
      Height = 29
      Caption = #1588#1605#1575#1585#1607' '#1662#1584#1610#1585#1588':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label6: TLabel
      Left = 503
      Top = 11
      Width = 94
      Height = 29
      Caption = #1588#1605#1575#1585#1607' '#1662#1584#1610#1585#1588':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label8: TLabel
      Left = 264
      Top = 10
      Width = 35
      Height = 27
      Caption = #1705#1575#1585#1576#1585':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label1: TLabel
      Left = 269
      Top = 48
      Width = 43
      Height = 28
      Caption = #1575#1586' '#1578#1575#1585#1610#1582':'
    end
    object Edit1: TEdit
      Left = 608
      Top = 9
      Width = 162
      Height = 37
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
    end
    object RadioButton1: TRadioButton
      Left = 415
      Top = 51
      Width = 68
      Height = 17
      Caption = #1576#1586#1585#1711#1587#1575#1604
      Checked = True
      TabOrder = 1
      TabStop = True
    end
    object RadioButton2: TRadioButton
      Left = 337
      Top = 51
      Width = 66
      Height = 17
      Caption = #1582#1585#1583#1587#1575#1604
      TabOrder = 2
    end
    object Edit2: TEdit
      Left = 608
      Top = 49
      Width = 162
      Height = 37
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 3
    end
    object Edit3: TEdit
      Left = 336
      Top = 9
      Width = 162
      Height = 37
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 4
    end
    object BBtnAccept: TBitBtn
      Left = 8
      Top = 91
      Width = 865
      Height = 24
      Caption = #1580#1587#1578#1580#1608
      TabOrder = 5
      OnClick = BBtnAcceptClick
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
    end
    object edtUserID: TEdit
      Left = 221
      Top = 7
      Width = 41
      Height = 34
      Color = clInfoBk
      Enabled = False
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 6
    end
    object edtUserName: TEdit
      Left = 46
      Top = 7
      Width = 176
      Height = 34
      Color = clInfoBk
      Enabled = False
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 7
    end
    object btnUser: TBitBtn
      Left = 10
      Top = 7
      Width = 35
      Height = 33
      Enabled = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 8
      OnClick = btnUserClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00}
    end
    object edtDate: TMaskEdit
      Left = 168
      Top = 44
      Width = 99
      Height = 37
      Color = clInfoBk
      Enabled = False
      EditMask = '1000/00/00;1;_'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      MaxLength = 10
      ParentFont = False
      TabOrder = 9
      Text = '1   /  /  '
    end
  end
  object MainMenuUnit: TMainMenu
    Left = 40
    Top = 231
    object MnuFile: TMenuItem
      Caption = #1662#1585#1608#1606#1583#1607
      object MnuEdit: TMenuItem
        Caption = #1608#1610#1585#1575#1610#1588
      end
      object MnuDelete: TMenuItem
        Caption = #1581#1584#1601
      end
      object N2: TMenuItem
        Caption = '-'
      end
      object MnuReturn: TMenuItem
        Caption = #1576#1575#1586#1711#1588#1578
      end
    end
  end
  object PopupMenuUnit: TPopupMenu
    Left = 73
    Top = 231
    object MnuPopEdit: TMenuItem
      Caption = #1608#1610#1585#1575#1610#1588
    end
    object MnuPopNone1: TMenuItem
      Caption = '-'
    end
    object MnuPopDelete: TMenuItem
      Caption = #1581#1584#1601
    end
  end
  object dsCredit: TDataSource
    DataSet = QCredit
    Left = 112
    Top = 232
  end
  object QCredit: TADOQuery
    Connection = DM.ADOConnection
    CursorType = ctStatic
    OnCalcFields = QCreditCalcFields
    Parameters = <
      item
        Name = 'UserID'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end
      item
        Name = 'ReceptionDate'
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 10
        Value = Null
      end>
    SQL.Strings = (
      
        'select CreditID, TCredit.ReceptionID, TCredit.Credit, ChargeTime' +
        ', Tcredit.UserID, CustomerName,'
      'StartTime, Keynumber, PositionClosed'
      
        'from TCredit inner join TReception on TCredit.ReceptionID=Trecep' +
        'tion.ReceptionID'
      'inner join TClosed on TReception.ClosedID=TClosed.ClosedID'
      
        'where TCredit.UserID= :UserID and TReception.ReceptionDate >= :R' +
        'eceptionDate'
      'order by CreditID'
      '')
    Left = 148
    Top = 232
    object QCreditCreditID: TLargeintField
      FieldName = 'CreditID'
    end
    object QCreditReceptionID: TLargeintField
      FieldName = 'ReceptionID'
    end
    object QCreditCredit: TIntegerField
      FieldName = 'Credit'
    end
    object QCreditChargeTime: TStringField
      FieldName = 'ChargeTime'
      FixedChar = True
      Size = 5
    end
    object QCreditUserID: TIntegerField
      FieldName = 'UserID'
    end
    object QCreditCustomerName: TWideStringField
      FieldName = 'CustomerName'
      Size = 100
    end
    object QCreditStartTime: TStringField
      FieldName = 'StartTime'
      FixedChar = True
      Size = 5
    end
    object QCreditKeynumber: TIntegerField
      FieldName = 'Keynumber'
    end
    object QCreditPositionClosed: TSmallintField
      FieldName = 'PositionClosed'
    end
    object QCreditstrPositionClosed: TStringField
      FieldKind = fkCalculated
      FieldName = 'strPositionClosed'
      Size = 6
      Calculated = True
    end
  end
  object ActionList: TActionList
    Left = 40
    Top = 264
    object ActionEdit: TAction
      Caption = 'ActionEdit'
      ShortCut = 16453
    end
    object ActionDel: TAction
      Caption = 'ActionDel'
      ShortCut = 46
    end
    object ActionReturn: TAction
      Caption = 'ActionReturn'
      ShortCut = 27
    end
    object ActionPrint: TAction
      Caption = 'ActionPrint'
      ShortCut = 16464
    end
  end
  object QDeleteCredit: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'CreditID'
        Attributes = [paSigned]
        DataType = ftLargeint
        Precision = 19
        Size = 8
        Value = Null
      end>
    SQL.Strings = (
      'Delete TCredit'
      'where CreditID= :CreditID')
    Left = 147
    Top = 264
  end
  object QSum: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'UserID'
        Size = -1
        Value = Null
      end
      item
        Name = 'ReceptionDate'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      
        'select Count(CreditID) as Tedad, sum(TCredit.Credit) as SumCredi' +
        't'
      
        'from TReception inner join TCredit on TReception.ReceptionID=Tcr' +
        'edit.ReceptionID'
      'where TCredit.UserID= :UserID and ReceptionDate= :ReceptionDate')
    Left = 80
    Top = 264
  end
end
