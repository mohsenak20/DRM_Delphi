object frmInternetReception: TfrmInternetReception
  Left = 368
  Top = 116
  Width = 874
  Height = 595
  BiDiMode = bdRightToLeft
  Caption = #1662#1584#1610#1585#1588' '#1575#1610#1606#1578#1585#1606#1578#1610
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -16
  Font.Name = 'Koodak'
  Font.Style = []
  OldCreateOrder = False
  ParentBiDiMode = False
  Position = poDesktopCenter
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 28
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 858
    Height = 107
    Align = alTop
    TabOrder = 0
    DesignSize = (
      858
      107)
    object Label14: TLabel
      Left = 780
      Top = 17
      Width = 42
      Height = 26
      Anchors = [akTop, akRight]
      Caption = #1575#1586' '#1578#1575#1585#1610#1582':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label3: TLabel
      Left = 665
      Top = 17
      Width = 15
      Height = 26
      Anchors = [akTop, akRight]
      Caption = #1575#1604#1610
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object edtStartDate: TMaskEdit
      Left = 685
      Top = 13
      Width = 92
      Height = 34
      Anchors = [akTop, akRight]
      Color = clInfoBk
      EditMask = '1000/00/00;1;_'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      MaxLength = 10
      ParentFont = False
      TabOrder = 0
      Text = '1   /  /  '
    end
    object edtEndDate: TMaskEdit
      Left = 570
      Top = 13
      Width = 88
      Height = 34
      Anchors = [akTop, akRight]
      Color = clInfoBk
      EditMask = '1000/00/00;1;_'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      MaxLength = 10
      ParentFont = False
      TabOrder = 1
      Text = '1   /  /  '
    end
    object btnSearch: TBitBtn
      Left = 7
      Top = 68
      Width = 844
      Height = 32
      Anchors = [akTop, akRight]
      Caption = #1580#1587#1578#1580#1608
      TabOrder = 2
      OnClick = btnSearchClick
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 488
    Width = 858
    Height = 68
    Align = alBottom
    TabOrder = 1
    object BtnReturn: TBitBtn
      Left = 26
      Top = 14
      Width = 87
      Height = 36
      Caption = #1576#1575#1586#1711#1588#1578
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      OnClick = BtnReturnClick
      Glyph.Data = {
        }
    end
  end
  object DBGrid1: TDBGrid
    Left = 0
    Top = 107
    Width = 858
    Height = 381
    Align = alClient
    DataSource = dsInternetReception
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -16
    TitleFont.Name = 'Koodak'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'webReceptionType'
        Title.Caption = #1593#1606#1608#1575#1606
        Width = 215
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'serviceCenterTitle'
        Title.Caption = #1662#1575#1585#1705
        Width = 198
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'adult'
        Title.Caption = #1576#1586#1585#1711#1587#1575#1604
        Width = 66
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'child'
        Title.Caption = #1582#1585#1583#1587#1575#1604
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'sumNumber'
        Title.Caption = #1580#1605#1593' '#1585#1583#1610#1601
        Width = 84
        Visible = True
      end>
  end
  object QInternetReception: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'startDate'
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 10
        Value = Null
      end
      item
        Name = 'endDate'
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 10
        Value = Null
      end>
    SQL.Strings = (
      'select webReceptionType, ReceptionType, other, '
      
        'service_centers.title serviceCenterTitle, sum(NumberOfAdult) adu' +
        'lt,'
      
        'sum(NumberOfChild) child, sum(NumberOfAdult+NumberOfChild) sumNu' +
        'mber from TReception'
      'inner join TWebReception on TReception.other = TWebReception.id'
      
        'inner join service_centers on TReception.service_center_id = ser' +
        'vice_centers.id'
      
        'where ReceptionDate >= :startDate and ReceptionDate >= :endDate ' +
        '  and ReceptionType = 3'
      
        'group by webReceptionType, ReceptionType, other, service_centers' +
        '.title')
    Left = 56
    Top = 88
  end
  object dsInternetReception: TDataSource
    DataSet = QInternetReception
    Left = 56
    Top = 120
  end
end
