object frmReceptionCard: TfrmReceptionCard
  Left = 344
  Top = 116
  Width = 496
  Height = 563
  BiDiMode = bdRightToLeft
  Caption = #1705#1575#1585#1578#1607#1575
  Color = clBtnFace
  Font.Charset = ARABIC_CHARSET
  Font.Color = clWindowText
  Font.Height = -16
  Font.Name = 'Koodak'
  Font.Style = [fsBold]
  OldCreateOrder = False
  ParentBiDiMode = False
  Position = poDesktopCenter
  PixelsPerInch = 96
  TextHeight = 28
  object PanelButton: TPanel
    Left = 0
    Top = 482
    Width = 488
    Height = 47
    Align = alBottom
    BevelInner = bvLowered
    TabOrder = 0
    object BtnReturn: TBitBtn
      Left = 26
      Top = 6
      Width = 87
      Height = 36
      Caption = #1576#1575#1586#1711#1588#1578
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      Glyph.Data = {
        }
    end
  end
  object DBGrid1: TDBGrid
    Left = 0
    Top = 0
    Width = 488
    Height = 482
    Align = alClient
    DataSource = dsCard
    TabOrder = 1
    TitleFont.Charset = ARABIC_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -16
    TitleFont.Name = 'Koodak'
    TitleFont.Style = [fsBold]
    Columns = <
      item
        Expanded = False
        FieldName = 'CompanyID'
        Title.Alignment = taCenter
        Title.Caption = #1705#1583
        Width = 46
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CompanyName'
        Title.Alignment = taCenter
        Title.Caption = #1588#1585#1705#1578
        Width = 319
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NumberOfCard'
        Title.Alignment = taCenter
        Title.Caption = #1578#1593#1583#1575#1583' '#1705#1575#1585#1578
        Width = 77
        Visible = True
      end>
  end
  object QReceptionCard: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'StartReceptionDate'
        Size = -1
        Value = Null
      end
      item
        Name = 'EndReceptionDate'
        Size = -1
        Value = Null
      end
      item
        Name = 'UserID'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      
        'declare @StartReceptionDate char(10), @EndReceptionDate char(10)' +
        ', @UserID int'
      'set @StartReceptionDate= :StartReceptionDate'
      'set @EndReceptionDate= :EndReceptionDate'
      'set @UserID= :UserID'
      'if @UserID=1'
      'begin'
      
        '  select CompanyName, TBargain.CompanyID, Count(ReceptionID)as N' +
        'umberOfCard'
      
        '  from TCompany inner join TBargain on TCompany.CompanyID=TBarga' +
        'in.CompanyID'
      '  inner join TCard on TBargain.BargainID=TCard.BargainID'
      '  inner join TReception on TCard.CardID=TReception.CardID'
      '  or TCard.CardID=TReception.CardID2'
      '  or TCard.CardID=TReception.CardID3'
      '  where ReceptionDate>=@StartReceptionDate and GetMoney=1'
      '  and ReceptionDate<=@EndReceptionDate'
      '  Group By CompanyName, TBargain.CompanyID'
      'end'
      'else'
      'begin'
      
        'select CompanyName, TBargain.CompanyID, Count(ReceptionID)as Num' +
        'berOfCard'
      
        '  from TCompany inner join TBargain on TCompany.CompanyID=TBarga' +
        'in.CompanyID'
      '  inner join TCard on TBargain.BargainID=TCard.BargainID'
      '  inner join TReception on TCard.CardID=TReception.CardID'
      '  or TCard.CardID=TReception.CardID2'
      '  or TCard.CardID=TReception.CardID3'
      '  where ReceptionDate>=@StartReceptionDate and GetMoney=1'
      
        '  and ReceptionDate<=@EndReceptionDate  and TReception.UserID= @' +
        'UserID'
      '  Group By CompanyName, TBargain.CompanyID'
      'end'
      '')
    Left = 32
    Top = 25
  end
  object dsCard: TDataSource
    DataSet = QReceptionCard
    Left = 40
    Top = 88
  end
end
