object frmSubBargain: TfrmSubBargain
  Left = 221
  Top = 179
  Width = 790
  Height = 561
  BiDiMode = bdRightToLeft
  Caption = #1579#1576#1578' '#1705#1575#1585#1578#1607#1575#1610' '#1662#1582#1588' '#1588#1583#1607
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -16
  Font.Name = 'Koodak'
  Font.Style = []
  KeyPreview = True
  Menu = MainMenuUnit
  OldCreateOrder = False
  ParentBiDiMode = False
  Position = poDesktopCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  DesignSize = (
    774
    503)
  PixelsPerInch = 96
  TextHeight = 28
  object PanelButton: TPanel
    Left = 0
    Top = 456
    Width = 774
    Height = 47
    Align = alBottom
    BevelInner = bvLowered
    TabOrder = 0
    object BtnNew: TBitBtn
      Left = 674
      Top = 7
      Width = 87
      Height = 36
      Caption = #1583#1585#1580
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      OnClick = BtnNewClick
      Glyph.Data = {
        
        FF00FFFFFF00FFFFFF00FFFFFF00}
    end
    object BtnEdit: TBitBtn
      Left = 546
      Top = 8
      Width = 87
      Height = 36
      Caption = #1608#1610#1585#1575#1610#1588
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      OnClick = BtnEditClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00}
    end
    object BtnDel: TBitBtn
      Left = 434
      Top = 8
      Width = 87
      Height = 36
      Caption = #1581#1584#1601
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 2
      OnClick = BtnDelClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00D8E9EC00}
    end
    object BtnReturn: TBitBtn
      Left = 26
      Top = 6
      Width = 87
      Height = 36
      Caption = #1576#1575#1586#1711#1588#1578
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 3
      OnClick = BtnReturnClick
      Glyph.Data = {
        }
    end
    object BtnPrint: TBitBtn
      Left = 318
      Top = 7
      Width = 87
      Height = 36
      Caption = #1670#1575#1662' '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 4
      OnClick = BtnPrintClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00}
    end
  end
  object StatusBar: TStatusBar
    Left = 0
    Top = 437
    Width = 774
    Height = 19
    Anchors = [akRight, akBottom]
    BiDiMode = bdRightToLeft
    Color = clInfoBk
    Panels = <
      item
        Text = #1576#1575#1586#1711#1588#1578' : Esc'
        Width = 139
      end
      item
        Text = #1670#1575#1662': Ctrl+P'
        Width = 139
      end
      item
        Text = #1581#1584#1601':  Del'
        Width = 139
      end
      item
        Text = #1608#1610#1585#1575#1610#1588':   Ctrl+E'
        Width = 139
      end
      item
        Text = #1583#1585#1580':  Insert'
        Width = 139
      end>
    ParentBiDiMode = False
  end
  object Grid: TDBGrid
    Left = 0
    Top = 115
    Width = 774
    Height = 322
    TabStop = False
    Align = alClient
    BiDiMode = bdRightToLeft
    Color = clInfoBk
    DataSource = dsSubBargain
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Traffic'
    Font.Style = [fsBold]
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete]
    ParentBiDiMode = False
    ParentFont = False
    ParentShowHint = False
    PopupMenu = PopupMenuUnit
    ShowHint = False
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'SubBargainID'
        Title.Caption = #1705#1583
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 62
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'BargainNumber'
        Title.Caption = #1588#1605#1575#1585#1607' '#1602#1585#1575#1585#1583#1575#1583
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 85
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'SubBargainCoName'
        Title.Caption = #1588#1585#1705#1578
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 87
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CompanyName'
        Title.Caption = #1583#1585#1610#1575#1601#1578' '#1705#1606#1606#1583#1607
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 234
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'StartCardNumber'
        Title.Caption = #1575#1586' '#1588#1605#1575#1585#1607
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 144
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'EndCardNumber'
        Title.Caption = #1575#1604#1610' '#1588#1605#1575#1585#1607
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Koodak'
        Title.Font.Style = []
        Width = 139
        Visible = True
      end>
  end
  object GroupBox: TGroupBox
    Left = 104
    Top = 278
    Width = 668
    Height = 159
    Anchors = [akRight, akBottom]
    Caption = #1605#1588#1582#1589#1575#1578' '
    Color = clBtnFace
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Traffic'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    TabOrder = 3
    Visible = False
    object Label4: TLabel
      Left = 451
      Top = 113
      Width = 21
      Height = 29
      Caption = #1575#1604#1610':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label6: TLabel
      Left = 604
      Top = 112
      Width = 55
      Height = 29
      Caption = #1575#1586' '#1588#1605#1575#1585#1607':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label8: TLabel
      Left = 606
      Top = 29
      Width = 50
      Height = 29
      Caption = #1602#1585#1575#1585#1583#1575#1583':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label1: TLabel
      Left = 599
      Top = 69
      Width = 49
      Height = 29
      Caption = #1588#1585#1705#1578':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object BtnSave: TBitBtn
      Left = 16
      Top = 24
      Width = 75
      Height = 36
      Caption = #1584#1582#1610#1585#1607
      TabOrder = 9
      OnClick = BtnSaveClick
      Glyph.Data = {
        
        00000000000000000000}
    end
    object BtnCancel: TBitBtn
      Left = 16
      Top = 72
      Width = 75
      Height = 36
      Caption = #1575#1606#1589#1585#1575#1601
      TabOrder = 10
      OnClick = BtnCancelClick
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
    end
    object edtStartCardNumber: TEdit
      Left = 477
      Top = 108
      Width = 121
      Height = 37
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 7
    end
    object edtEndCardNumber: TEdit
      Left = 325
      Top = 108
      Width = 121
      Height = 37
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 8
    end
    object edtBargainID: TEdit
      Left = 536
      Top = 28
      Width = 60
      Height = 37
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
    end
    object edtCompanyName: TEdit
      Left = 145
      Top = 28
      Width = 285
      Height = 37
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 2
    end
    object btnBargain: TBitBtn
      Left = 109
      Top = 29
      Width = 35
      Height = 33
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 3
      OnClick = btnBargainClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00}
    end
    object edtCoID: TEdit
      Left = 536
      Top = 68
      Width = 60
      Height = 37
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 4
    end
    object edtCoName: TEdit
      Left = 249
      Top = 68
      Width = 285
      Height = 37
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 5
    end
    object btnCo: TBitBtn
      Left = 213
      Top = 69
      Width = 35
      Height = 33
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 6
      OnClick = btnCoClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00}
    end
    object edtBargainNumber: TEdit
      Left = 432
      Top = 28
      Width = 101
      Height = 37
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 774
    Height = 115
    Align = alTop
    TabOrder = 4
    DesignSize = (
      774
      115)
    object Label2: TLabel
      Left = 671
      Top = 7
      Width = 50
      Height = 29
      Anchors = [akTop, akRight]
      Caption = #1602#1585#1575#1585#1583#1575#1583':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label3: TLabel
      Left = 673
      Top = 48
      Width = 29
      Height = 29
      Anchors = [akTop, akRight]
      Caption = #1607#1578#1604':'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Traffic'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object edtSBargainID: TEdit
      Left = 608
      Top = 4
      Width = 60
      Height = 37
      Anchors = [akTop, akRight]
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
    end
    object edtSCompanyName: TEdit
      Left = 217
      Top = 4
      Width = 285
      Height = 37
      Anchors = [akTop, akRight]
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
    end
    object btnSBargain: TBitBtn
      Left = 181
      Top = 4
      Width = 35
      Height = 33
      Anchors = [akTop, akRight]
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 2
      OnClick = btnSBargainClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00}
    end
    object edtHotelID: TEdit
      Left = 608
      Top = 44
      Width = 60
      Height = 37
      Anchors = [akTop, akRight]
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 3
    end
    object edtHotelName: TEdit
      Left = 321
      Top = 44
      Width = 285
      Height = 37
      Anchors = [akTop, akRight]
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 4
    end
    object btnHotel: TBitBtn
      Left = 285
      Top = 45
      Width = 35
      Height = 33
      Anchors = [akTop, akRight]
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 5
      OnClick = btnHotelClick
      Glyph.Data = {
        
        EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00D8E9EC00}
    end
    object edtSBargainNumber: TEdit
      Left = 504
      Top = 4
      Width = 101
      Height = 37
      Anchors = [akTop, akRight]
      Color = clInfoBk
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -17
      Font.Name = 'Koodak'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 6
    end
    object BBtnAccept: TBitBtn
      Left = 5
      Top = 85
      Width = 772
      Height = 25
      Anchors = [akTop, akRight]
      Caption = #1580#1587#1578#1580#1608
      TabOrder = 7
      OnClick = BBtnAcceptClick
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
    end
  end
  object MainMenuUnit: TMainMenu
    Left = 8
    Top = 15
    object MnuFile: TMenuItem
      Caption = #1662#1585#1608#1606#1583#1607
      object MnuInsert: TMenuItem
        Caption = #1583#1585#1580
        OnClick = BtnNewClick
      end
      object MnuEdit: TMenuItem
        Caption = #1608#1610#1585#1575#1610#1588
        OnClick = BtnEditClick
      end
      object MnuDelete: TMenuItem
        Caption = #1581#1584#1601
        OnClick = BtnDelClick
      end
      object N2: TMenuItem
        Caption = '-'
      end
      object mnuCheck: TMenuItem
        Caption = #1670#1705' '#1606#1605#1608#1583#1606' '#1575#1591#1604#1575#1593#1575#1578' '#1608#1575#1585#1583#1607
        OnClick = mnuCheckClick
      end
      object MnuReturn: TMenuItem
        Caption = #1576#1575#1586#1711#1588#1578
        OnClick = BtnReturnClick
      end
    end
  end
  object PopupMenuUnit: TPopupMenu
    Left = 41
    Top = 15
    object MnuPopInsert: TMenuItem
      Caption = #1583#1585#1580
      OnClick = BtnNewClick
    end
    object MnuPopEdit: TMenuItem
      Caption = #1608#1610#1585#1575#1610#1588
      OnClick = BtnEditClick
    end
    object MnuPopNone1: TMenuItem
      Caption = '-'
    end
    object MnuPopDelete: TMenuItem
      Caption = #1581#1584#1601
      OnClick = BtnDelClick
    end
  end
  object SP_SubBargain: TADOStoredProc
    Connection = DM.ADOConnection
    ProcedureName = 'SP_SubBargain;1'
    Parameters = <
      item
        Name = '@RETURN_VALUE'
        DataType = ftInteger
        Direction = pdReturnValue
        Precision = 10
        Value = Null
      end
      item
        Name = '@SubBargainID'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@BargainID'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@CoID'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@StartCardNumber'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@EndCardNumber'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@Action'
        Attributes = [paNullable]
        DataType = ftWideString
        Size = 6
        Value = Null
      end>
    Left = 77
    Top = 15
  end
  object dsSubBargain: TDataSource
    DataSet = QSubBargain
    Left = 112
    Top = 16
  end
  object QSubBargain: TADOQuery
    Connection = DM.ADOConnection
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      
        'select SubBargainID, TBargain.BargainID, BargainNumber, TCompany' +
        '.CompanyID, CompanyName, StartCardNumber, EndCardNumber,'
      'SubBargainCoID, SubBargainCoName'
      
        'from TSubBargain inner join TBargain on TSubBargain.BargainID=TB' +
        'argain.BargainID'
      'inner join TCompany on TBargain.CompanyID=TCompany.CompanyID'
      
        'inner join TSubBargainCo on TSubBargain.CoID=TSubBargainCO.SubBa' +
        'rgainCoID')
    Left = 148
    Top = 16
  end
  object ActionList: TActionList
    Left = 8
    Top = 48
    object ActionInsert: TAction
      Caption = 'ActionInsert'
      ShortCut = 45
      OnExecute = BtnNewClick
    end
    object ActionEdit: TAction
      Caption = 'ActionEdit'
      ShortCut = 16453
      OnExecute = BtnEditClick
    end
    object ActionDel: TAction
      Caption = 'ActionDel'
      ShortCut = 46
    end
    object ActionReturn: TAction
      Caption = 'ActionReturn'
      ShortCut = 27
      OnExecute = BtnReturnClick
    end
    object ActionPrint: TAction
      Caption = 'ActionPrint'
      ShortCut = 16464
    end
  end
  object QDelete: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'SubBargainID'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      'delete TSubBargain'
      'where SubBargainID= :SubBargainID')
    Left = 40
    Top = 48
  end
  object QPrint: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <
      item
        Name = 'SubBargainCoID'
        Size = -1
        Value = Null
      end
      item
        Name = 'BargainID'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'Declare @SubBargainCoID   int'
      'Declare @BargainID        int'
      'Set @SubBargainCoID = :SubBargainCoID'
      'Set @BargainID = :BargainID'
      ''
      ''
      'Declare @TTemp table'
      '('
      #9'SubBargainCoID'#9#9'int,'
      #9'SubBargainCoName'#9'NvarChar(100),'
      #9'GetCard'#9#9#9'int,'
      #9'UsedCard'#9#9'int'
      ')'
      ''
      ''
      'if @BargainID <> -1'
      'begin'
      '    if @SubBargainCoID <> -1'
      '    begin'
      '      insert into @TTemp'
      '      SELECT SubBargainCoID, SubBargainCoName,'
      
        '      (select sum(EndCardNumber-StartCardNumber+1) from TSubBarg' +
        'ain where CoID=TCo.SubBargainCoID and BargainID = @BargainID) as' +
        ' GetCard,'
      
        '      (select sum(NumberOfAdult+NumberOfChild) from TReception i' +
        'nner join TSubBargain on TReception.CardID<=TSubBargain.EndCardN' +
        'umber'
      
        '      and TReception.CardID>=StartCardNumber and TSubBargain.CoI' +
        'D=TCo.SubBargainCoID and BargainID = @BargainID) as UsedCard'
      '      from TSubBargainCo as TCO'
      '      where SubBargainCoID = @SubBargainCoID'
      '      order by GetCard DESC'
      '    end'
      '    else'
      '    begin'
      '      insert into @TTemp'
      '      SELECT SubBargainCoID, SubBargainCoName,'
      
        '      (select sum(EndCardNumber-StartCardNumber+1) from TSubBarg' +
        'ain where CoID=TCo.SubBargainCoID and BargainID = @BargainID) as' +
        ' GetCard,'
      
        '      (select sum(NumberOfAdult+NumberOfChild) from TReception i' +
        'nner join TSubBargain on TReception.CardID<=TSubBargain.EndCardN' +
        'umber'
      
        '      and TReception.CardID>=StartCardNumber and TSubBargain.CoI' +
        'D=TCo.SubBargainCoID and BargainID = @BargainID) as UsedCard'
      '      from TSubBargainCo as TCO'
      '      order by GetCard DESC'
      '    end'
      'end'
      'else'
      'begin'
      '    if @SubBargainCoID <> -1'
      '    begin'
      '      insert into @TTemp'
      '      SELECT SubBargainCoID, SubBargainCoName,'
      
        '      (select sum(EndCardNumber-StartCardNumber+1) from TSubBarg' +
        'ain where CoID=TCo.SubBargainCoID ) as GetCard,'
      
        '      (select sum(NumberOfAdult+NumberOfChild) from TReception i' +
        'nner join TSubBargain on TReception.CardID<=TSubBargain.EndCardN' +
        'umber'
      
        '      and TReception.CardID>=StartCardNumber and TSubBargain.CoI' +
        'D=TCo.SubBargainCoID) as UsedCard'
      '      from TSubBargainCo as TCO'
      '      where SubBargainCoID = @SubBargainCoID'
      '      order by GetCard DESC'
      '    end'
      '    else'
      '    begin'
      '      insert into @TTemp'
      '      SELECT SubBargainCoID, SubBargainCoName,'
      
        '      (select sum(EndCardNumber-StartCardNumber+1) from TSubBarg' +
        'ain where CoID=TCo.SubBargainCoID ) as GetCard,'
      
        '      (select sum(NumberOfAdult+NumberOfChild) from TReception i' +
        'nner join TSubBargain on TReception.CardID<=TSubBargain.EndCardN' +
        'umber'
      
        '      and TReception.CardID>=StartCardNumber and TSubBargain.CoI' +
        'D=TCo.SubBargainCoID) as UsedCard'
      '      from TSubBargainCo as TCO'
      '      order by GetCard DESC'
      '    end'
      'end'
      ''
      ''
      
        'select SubBargainCoID, SubBargainCoName, GetCard, UsedCard, Used' +
        'Card*100/GetCard as PercentUsedCard, '
      'GetCard-UsedCard as NotUsed from @TTemp')
    Left = 8
    Top = 88
  end
  object QTempForCheck: TADOQuery
    Connection = DM.ADOConnection
    Parameters = <>
    Left = 56
    Top = 88
  end
end
